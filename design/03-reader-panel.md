...
ReaderPanel 不能用样例报告填补真实报告缺失。所有入口先判断当前上下文,再进入报告、运行态或空态:

| 空态 | 表现 | 主动作 |
|---|---|---|
| 无可读章节 | 标题「还没有可预演的章节」,说明当前只打开了设定、大纲或空纸面 | 打开章节 / 让 AI 起草一章 |
| 章节太短 | 标题「章节太短,读者还没进入状态」,说明最低可读长度来自默认值登记 | 继续写作 / 稍后再跑 |
| 没有最近 ReaderPanel 报告 | 标题「还没有最近报告」,说明「打开最近报告」不会伪造旧结果 | 运行 ReaderPanel |

段落定位必须保持作者当前位置可恢复:点击风险、亮点、persona chip 或叙事诊断证据里的「第 N 段」后,编辑器滚动到对应段落,用 2s accent 边框或背景提示目标段;ReaderPanel 面板不关闭,风险勾选、展开状态和行动栏选择不丢。定位失败时显示「段落锚点已变化,需要重跑报告」,不跳到相邻段猜测。
...
- 取消:已完成 persona 保留;<3 成功 → 头部显示 `insufficient`,不出分类建议,提供「补跑失败的 2 个」按钮(最少成功数默认值引用见 [A04 §用户可见默认值登记](../spec/appendix/A04-tool-catalog.md#用户可见默认值登记))
