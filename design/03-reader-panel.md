...
5 类模拟读者(追更党 / 逻辑控 / 情感党 / 毒舌读者 / 潜水大佬)并行读一章,聚合成一份**发布前风险预演报告**。报告可嵌入写作审批卡,也可从统一搜索命令候选或报告面板入口触发后以读者报告面板打开。

入口有两类:「运行读者预演」针对当前章节或选区发起预演;「打开最近读者报告」只打开已有报告。若当前没有可读章节、章节过短或没有最近报告,入口展示空态和恢复动作,不伪造报告。

## 报告信息架构

```mermaid
flowchart TB
  HEAD["头部: 分类风险摘要 + 多人共识徽标 + 5 类模拟读者 色点"]
...
- **分类风险摘要**:展示「风险低 / 需留意 / 风险集中 / 证据不足」四类信号(枚举与语义主权见 [spec/M11 §风险信号枚举](../spec/M11-reader-panel.md#风险信号枚举):不打总分、不出发布结论、按多人共识与证据呈现);状态旁列出最主要的 1-2 个风险类别
...
读者预演不能用样例报告填补真实报告缺失。所有入口先判断当前上下文,再进入报告、运行态或空态:

| 空态 | 表现 | 主动作 |
|---|---|---|
| 无可读章节 | 标题「还没有可预演的章节」,说明当前只打开了设定、大纲或空纸面 | 打开章节 / 让 AI 起草一章 |
| 章节太短 | 标题「章节太短,读者还没进入状态」,说明最低可读长度来自默认值登记 | 继续写作 / 稍后再跑 |
| 没有最近读者报告 | 标题「还没有最近报告」,说明「打开最近报告」不会伪造旧结果 | 运行读者预演 |

段落定位必须保持作者当前位置可恢复:点击风险、亮点、读者标签 或叙事诊断证据里的「第 N 段」后,编辑器滚动到对应段落,用 2s accent 边框或背景提示目标段;读者报告面板不关闭,风险勾选、展开状态和行动栏选择不丢。定位失败时显示「段落锚点已变化,需要重跑报告」,不跳到相邻段猜测。

## 叙事诊断区块

叙事诊断是读者报告里的结构化体检层,用于回答「这一章在爽点、情绪、因果和悬念上哪里变弱」。它不替代 5 类模拟读者的真实读者反应,也不输出发布结论;它给出可见证据、历史对照和可重跑入口,供作者决定是否修改。
...
- 自定义读者卡右上角 `badge-neutral「自定义」`;编辑入口跳 设置面板「读者预演」

## 进行态(长任务)

- 触发后状态点旁显示进度:「读者预演 · 3/5 · 毒舌读者 · 4.5s」([plan/07 §长任务体验](../plan/07-collaboration-and-modes.md#长任务体验))
- 报告区先以骨架卡占位,每个 persona 完成即点亮其色点并填入缩略 sentiment
- 取消:已完成读者反馈保留;<3 成功 → 头部显示 `insufficient`,不出分类建议,提供「补跑失败的 2 个」按钮(最少成功数默认值引用见 [A04 §用户可见默认值登记](../spec/appendix/A04-tool-catalog.md#用户可见默认值登记))
...
|---|---|
| 全部成功 | 完整报告 |
| 部分失败(≥3 成功) | 正常聚合;失败读者色点空心 + tooltip 错误摘要 + 单个重跑 |
| <3 成功 | `insufficient` 徽标,只列已完成反馈,无分类建议 |
| 章节过短(<800 字) | 空态:「章节太短,读者还没进入状态」+ 继续写作引导(阈值默认值引用见 [A04 §用户可见默认值登记](../spec/appendix/A04-tool-catalog.md#用户可见默认值登记)) |
| 嵌在审批卡内 | 报告作为卡内一个区块,行动钮「按风险点修改」= 将已勾风险转入拒绝反馈 |

「按风险点修改」有两条出路:

- 嵌在审批卡内:已勾风险行的原因、段落位置和读者共识转入拒绝反馈框,提交后进入拒绝反馈环([design/02 §行动栏](./02-approval-cascade.md#行动栏))。
- 独立读者报告面板:已勾风险行转成润色清单预填项,发起一次润色修改提案;每个预填项保留来源段落,生成结果仍需审定。
