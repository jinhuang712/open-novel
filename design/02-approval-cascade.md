...
整个修改包(主修改 + 1~3 级连带影响)在**一张卡**里一次审完;这是产品最重的交互,层级必须一眼可读:**发生了什么 → 波及多大 → 有什么风险 → 我怎么决定**。作者界面主标签使用「审批卡」「整批审」「连带修改」,不展示内部工具名、文件路径或锚点短码。

## 信息架构

```mermaid
flowchart TB
  H["卡头: ✱ 写手想修改林溪角色卡<br/>+8 项连带修改 · 3 轮分析 · 原因一句话"]
...
| 标题 | 作者可读位置,如「大纲 · 第 3 段」「第 12 章 · 第 5 段」「林溪角色卡」。真实作者包不显示 `.md`、相对路径或锚点短码。 |
...
每条风险可点击 → 跳对应章节段(anchor 跳转,与实体跳转同机制)。write 模式章节类工具时,此区域上方再嵌读者预演章节风险报告(见 [design/03](./03-reader-panel.md))。
...
- 点击「返回审批卡」、状态点待审批文案或底部条本身,回到完整审批卡,并恢复先前滚动位置和勾选状态。
- 若从读者预演风险行进入,底部条追加「来自读者预演」来源 chip;返回后该风险行保持选中。

这一路径与 [design/01 §对照视图](./01-main-layout.md#对照视图) 共用双 pane 心智,但审批主权仍属于审批卡。
