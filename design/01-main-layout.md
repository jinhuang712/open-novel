...
- 进入:章节轨拖到纸面右半、库面板/最近列表选择「对照打开」、统一搜索 / 快速打开使用 `Cmd+Enter`、实体「打开定义」或审批风险「跳对应章节段」。
...
- 结构:标题行(动作摘要 + 作品内位置小字,如「林溪角色卡 · 写手」)→ diff 块 → 连带影响提示行(影响数 + 「去整批审 →」[design/02](./02-approval-cascade.md))→ 操作行:同意(primary)/ 编辑后同意 / 拒绝(danger ghost),`Y/E/N` 仅在卡片聚焦后直达
