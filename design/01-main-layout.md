...
- 进入:章节轨拖到纸面右半、库面板/最近列表选择「对照打开」、统一搜索使用 `Cmd+Enter`、实体「打开定义」或审批风险「跳对应章节段」。
...
- 形态:屏幕上 1/4 处居中 720px(`--bg-raised` + 1px 边 + `--shadow-md`):单行输入 → 左侧分组结果(角色 / 阵营 / 概念 / 章节 / 事实答案 / 引用 / 命令 / 可能相关)→ 右侧 hover preview、答案证据或命令说明
...
- 与 spec 对齐:行为主权见 [spec/M01 Universal Search](../spec/M01-universal-search.md);本篇只定交互形态和视觉层级。作者界面主标签写「统一搜索」,不显示英文能力名。
...
- **规则风险**:红色虚线下划线,hover 旁注红色语义(「此世界不存在」+ 建议改写);汇总 = 左下 ⚠ 微标 + 滚动条 marker;无段落 gutter 图标(见 [spec/S14](../spec/S13-editor-and-interaction.md))
...
一粒 8px 的可见点,固定右下角 20px;命中容器 24px。一句话在点左侧浮现(12px,`--text-secondary`),只承载作者当前最需要处理的三种状态:

| 态 | 点 | 旁文 | 点击 |
|---|---|---|---|
| 空闲 | `--text-tertiary` | 无;hover 浮现「✦ 对话 Cmd+L」 | 召出输入条 |
| 运行中 | `--accent`,2.4s 透明度呼吸(全应用唯一循环动效;`prefers-reduced-motion` 下静止) | 「写手正在整理修改建议 · 12s」+ 进度 `3/5 · 毒舌读者` + 取消 | 展开 Trace 面板 |
| 待审批 | `--accent`,静止 | 「1 个修改待审批 — 查看」(`--accent-text`) | 弹审批聚焦卡 |

停止、连接失败和凭据失效不作为状态点常驻枚举。停止结果写入最近回执和过程面板;连接失败用右上 toast 提醒并给「打开设置」入口,设置面板内再处理连接凭据。

过程面板(`Cmd+J` / 点状态点,380px 推开式):头部 = 最近回执摘要 +「折叠全部」;主体按 AI 角色分块——角色名 + 耗时一行,工具调用行等宽可展开明细,reasoning 默认一句摘要。真实用户包不显示 token、缓存命中、用量或「复制过程」;开发构建可用只读诊断面板展示全文记录,由构建/打包变量控制。块间发丝线分隔,不做卡片底色。
...
- 占位文案按模式变化:讨论 = 「讨论模式不会修改任何文件,可 @引用角色/设定文件/章节」;规划 = 「规划模式只生成设定和结构方案,可 @引用角色/设定文件/章节」;写作 = 「写作模式会生成正文修改提议,可 @引用角色/设定文件/章节」。
...
- 结构:标题行(动作摘要 + 作品内位置小字,如「林溪角色卡 · 写手」)→ 修改对照块 → 连带影响提示行(影响数 + 「去整批审 →」[design/02](./02-approval-cascade.md))→ 操作行:同意(primary)/ 编辑后同意 / 拒绝(danger ghost),`Y/E/N` 仅在卡片聚焦后直达
...
- 焦点恢复:打开审批卡前记录关闭前拥有焦点的元素和编辑器锚点。关闭(×/`Esc`)后优先把焦点还给该元素;若原元素已不存在或不可聚焦,焦点回到右下状态点「1 个修改待审批 — 查看」。自动弹出审批卡不抢编辑器焦点,但用户主动进入卡片后,关闭仍按同一规则恢复,屏幕阅读器播报「审批暂不处理,仍待审批」。
...
| 60 | 统一搜索 / @引用 | 轻浮层 | 同组互斥,后打开者关闭先打开者;命令候选与打开结果并入统一搜索 |
...
| 停止无改动 | 状态点回到空闲;右上 toast 提示「已停止,没有修改」;过程面板顶部列出已完成结果、未完成步骤和继续入口 |
| 连接失败 / 凭据失效 | 状态点不切错误态;右上 toast 提示「连接失败,去设置检查凭据」并提供设置入口;输入条顶部可显示同一句阻断说明 |
...
| 主题切换 | 200ms | 设置面板「外观」切换后背景/文字色过渡 |

此外没有任何动画:无涟漪、无弹跳、无视差、无骨架闪光(骨架屏为静态灰条)。

## Toast

Toast 固定在右上方,距离窗口右边 24px、顶部工具栏下沿约 20px;不居中悬浮,不遮正文主阅读区。同屏最多 3 条,4s 自动消退,hover 暂停。toast 只做结果与入口提示,不提供即时撤销;需要改变作品状态的修正都进入新的审定动作。该 placement 与 [design/06 §Toast](./06-command-palette.md#toast) 共用。
...
- 入口只在设置面板「外观」分区;主界面顶部、章节轨、状态点和过程面板都不放主题切换。
...
- **全项目改名(F2 / 右键)**:弹一行输入(预填当前名、全选);确认后不直接写盘——按引用索引找出全项目所有出现位置,生成整批修改建议走审批,逐文件展示,可整批同意或逐条处理([design/02](./02-approval-cascade.md))
