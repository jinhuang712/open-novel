...
| 6 文风 | 📂 | 范文、风格偏好、Humanizer 经验入口 |
| 7 创作守则 | 📂 | 五大守则只读展示和必要说明 |
| 8 记忆 | 📂 | 经验可见、权重 0-5、来源和删除 |
| 9 用量 | 🌐 | token 消耗、缓存命中、上下文峰值等只读指标 |
| 10 快捷键 | 🌐 | 快捷键查看与重绑 |
| 11 关于 | 🌐 | 版本、隐私边界和 dev build 只读标记 |
...
- **连接凭据**:masked 输入 + 显隐 toggle +「测试连接」(loading → ✓ 已验证 success / ✗ 失败 danger + 原因)。只处理连接是否可用、凭据是否已验证和失败原因;不展示本月用量、缓存命中或上下文峰值。字段对作者显示为「连接凭据」,不显示 API Keys。
- **外观**:独立分区,包含跟随系统 / 浅色 / 深色三段控件、提高对比度、字号密度预览和主题说明。设计 token 中的高对比与主题归这里,不混入「文风」或「助理语气」。切换即时生效;统一搜索命令候选可搜「打开外观设置」,但不直接切换主题。提高对比度开关写入外观设置,应用到根节点 `data-contrast="more"`;关闭时移除该属性,只改变颜色 token,不改变字号、间距和布局。
...
- **文风**:展示范文入口、表达偏好和 Humanizer 相关经验摘要。它只影响表达层建议和润色倾向,不改变剧情事实、写入权限或 ReaderPanel 读者类型。
...
- **用量**:只读展示本月 token、缓存命中、上下文峰值和最近失败摘要。它不提供账单管理、数据清理、凭据录入或项目导出入口。
- **快捷键**:表格(命令 / 默认键 / 当前键 / 重绑按钮);重绑进入捕获态「按下新组合键…」,冲突即时红字提示并禁止保存;`Esc` 退出捕获(不可绑 Esc,[spec/S14](../spec/S13-editor-and-interaction.md))
- **关于**:显示版本、数据只在本机、隐私边界和当前构建标记。真实用户包不显示开发模式开关;若是 dev build,只显示「开发构建:诊断信息可见」说明,不可在普通设置里开启。
