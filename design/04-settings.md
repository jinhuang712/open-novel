...
- **外观**:独立分区,包含跟随系统 / 浅色 / 深色三段控件、提高对比度、字号密度预览和主题说明。设计 token 中的高对比与主题归这里,不混入「文风」或「助理语气」。切换即时生效;命令面板可搜「切换深浅外观」。提高对比度开关写入外观设置,应用到根节点 `data-contrast="more"`;关闭时移除该属性,只改变颜色 token,不改变字号、间距和布局。
