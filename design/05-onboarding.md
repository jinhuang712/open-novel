...
首启 = 品牌时刻:全屏素色底 + 居中 620px 卡片,标题用衬线,是整个产品里书卷气最重的一页。目标 3 分钟进入主界面。首启首先确认 workspace,因为项目事实、导入恢复和权限检查都从这个路径开始。

## 向导骨架

```mermaid
flowchart LR
  S1["1 Workspace<br/>选择 / 创建路径"] --> S2["2 API Key<br/>测试连接 / Mock"] --> S3["3 第一个项目<br/>样例 / 空白 / 导入"] --> S4["4 三件事<br/>核心心智"] --> MAIN([进入主界面])
  S1 -. 跳过全部 .-> MAIN
```
...
| 3 第一个项目 | 三分支:推荐卡「加载样例项目〈重生互联网〉」(badge-accent「推荐 · 5 分钟看懂全流程」),空白表单(项目名 / 流派下拉 / 风格描述 / 故事种子 textarea),导入项目包 | 三选一卡片;选空白时表单就地展开,项目名必填。选导入时展示文件选择、manifest 预览、冲突处理(另存副本 / 覆盖恢复 / 取消)。导入校验失败停留本步,不写入项目列表。 |
...
| 导入 manifest 不兼容 | 导入卡 danger 条说明需要的应用/格式版本,不猜测包结构,不创建项目 |
| 导入文件冲突 | 显示同名 project id / 标题 / 导出时间,默认选中「另存副本」;覆盖恢复必须进入恢复预览 |
| 导入索引失败 | 项目可进入,顶部 warning「索引过期,已排队修复」;搜索能力标记为 degraded |
