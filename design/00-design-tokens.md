# design/00 — Design Tokens(雾绿 · 嫩叶主题 · 双主题)
...
- **雾绿中性而非纯白/纯黑**:浅色底是雾绿白 `#F5F8F4`,深色底是墨绿炭 `#1C211D`;全部中性灰都带极轻的绿倾向,与品牌色同源
- **低饱和 + 单一品牌 accent**:界面大面积雾绿灰,唯一高饱和色是嫩叶绿 `#158243`(深色主题 `#3DA066`),只用于主操作、品牌时刻与选中态
...
| Token | Light | Dark | 用途 |
|---|---|---|---|
| `--bg-app` | `#F5F8F4` | `#1C211D` | 应用底色 |
| `--bg-surface` | `#FBFCFA` | `#242A25` | 卡片 / 编辑器纸面(非纯白) |
| `--bg-sunken` | `#ECF1EC` | `#161A17` | 侧栏 / 输入底 / 下沉区 |
| `--bg-raised` | `#FDFEFD` | `#2C342D` | 浮层 / 弹窗 |
| `--bg-hover` | `rgba(47,68,56,.05)` | `rgba(212,226,217,.06)` | hover 态 |
| `--bg-active` | `rgba(47,68,56,.10)` | `rgba(212,226,217,.11)` | active / 选中底 |
| `--bg-overlay` | `rgba(20,28,23,.40)` | `rgba(4,8,6,.55)` | 模态遮罩 |
| `--text-primary` | `#3F5148` | `#B9C6BF` | 正文(实测 7.9 / 8.3,落在 7–9 区间) |
| `--text-secondary` | `#5C6F66` | `#8B9A93` | 次要说明(实测 5.0 / 5.0) |
| `--text-tertiary` | `#7B8C84` | `#6F7A75` | 占位 / 弱提示(实测 3.3 / 3.3) |
| `--border` | `#E1E8E2` | `#333B35` | 默认描边 |
| `--border-strong` | `#CBD7CE` | `#46524A` | 分隔强描边 / 控件描边 |
...
| Agent | `--agent-router` | `#7E8B80` | `#889287` | Router(ThinkingPanel 主题色) |
