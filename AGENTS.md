...
- `spec/*.md`: 核心技术文档(实施向);根层 `spec/00-11` 写系统契约、技术路径、职责边界和失败语义,实现明细归 `spec/appendix/`。
...
spec/ 是技术设计契约,不是实现明细堆放处。核心 spec 必须先讲清系统怎么分层、能力如何协作、职责边界在哪里,重点是技术路径、设计取舍、状态流转、失败语义和用户可见结果。
...
- `plan/*.md`: 纯产品 PRD——产品设计、原则、红线、目标与边界、核心能力;零技术细节。
- `spec/*.md`: 实现契约、技术路径、职责边界、失败语义和必要接口;表结构、完整 schema、代码片段、枚举、prompt 与测试矩阵归 `spec/appendix/`。
