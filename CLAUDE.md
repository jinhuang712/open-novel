...
- `spec/*.md`: 核心技术文档(实施向);根层 `spec/00-11` 必须写可直接阅读的系统主权设计,`spec/12+` 承载用户可感知能力的完整契约;实现明细归 `spec/appendix/`。
...
根层 `spec/00-11` 不允许写成同一个模板的换皮文档。每篇应按主题选择自己的阅读骨架:系统地图、事故剧本、生命周期、状态机、泳道、证据包、质检室、交互路由或控制面板都可以,但不能机械套用“问题 / 主权对象 / 流程 / 失败语义”的固定段落。

用户可感知且可独立实现/测试的能力应直接进入根层编号 spec(`spec/12+`),不要另建二级能力目录。Universal Search、Discuss Mode、Trace、Approval Cascade、ReaderPanel 这类能力都属于核心 spec。
...
- `plan/*.md`: 纯产品 PRD——产品设计、原则、红线、目标与边界、核心能力;零技术细节。
- `spec/*.md`: 实现契约、技术路径、职责边界、失败语义、用户可见结果和必要接口;根层 spec 必须按主题采用不同骨架并用图表/表格/FAQ 讲清设计,用户可感知能力直接用根层编号 spec 承载,表结构、完整 schema、代码片段、枚举、prompt 与测试矩阵归 `spec/appendix/`;旧原文和阶段历史归 `progress/`。
