...
- `spec/*.md`: 核心技术文档(实施向);根层 `Sxx/Mxx/Ixx/Rxx` 分别承载系统设计、用户能力、集成契约和可靠性/运维契约;实现与验证明细归 `spec/appendix/Axx` / `spec/appendix/Vxx`。
...
根层 `spec/Sxx` / `spec/Mxx` / `spec/Ixx` / `spec/Rxx` 不允许写成同一个模板的换皮文档。每篇应按主题选择自己的阅读骨架:系统地图、事故剧本、生命周期、状态机、泳道、证据包、质检室、交互路由、集成边界或恢复手册都可以,但不能机械套用“问题 / 主权对象 / 流程 / 失败语义”的固定段落。

用户可感知且可独立实现/测试的能力应直接进入根层编号 spec(`spec/Mxx`),不要另建二级能力目录。Universal Search、Discuss Mode、Trace、Approval Cascade、ReaderPanel 这类能力都属于核心 spec。跨边界接入用 `spec/Ixx`,运行维护和恢复闭环用 `spec/Rxx`,不要混进 appendix。
...
- `spec/I*.md`: 模型、编辑器、文件系统、导入导出、桌面壳等跨边界集成契约。
- `spec/R*.md`: 项目生命周期、备份恢复、迁移升级、索引修复、诊断排障等可靠性/运维契约。
