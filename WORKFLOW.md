...
| 是否有未关闭裁决、无主权归口风险或实施期验证 | 裁决/无归口风险进 `TODO.md`;实施期验证进 `spec/appendix/Vxx` |
...
清空 TODO 不是把问题删掉,而是把每个问题迁入稳定主权位置后,让 TODO 不再承担已裁决事实或已归口验证项的索引职责。

执行顺序:

1. 先逐条判断 TODO 的真实归口:产品承诺归 `plan/`,技术契约归 `spec/Sxx|Mxx`,平台支撑归 `spec/platform/Ixx|Rxx`,机器明细和验证证据归 `spec/appendix/Axx|Vxx`,交互归 `design/`。
2. 已有用户裁决或可直接关闭的项,先迁入对应主权文档;仍缺用户选择、没有主权文档承接或无法写清失败收场的项继续留在 `TODO.md`。
3. 真实工程验证如果需要可运行应用、fixture、外部服务或 spike harness,文档阶段只记录验证归口、实施期触发点、关闭条件和失败后回写位置;一旦这些内容已由 `Vxx` 稳定承接,不再作为游离 TODO 保留。
4. 迁移后搜索并替换旧 TODO 编号反链。正文引用应改成稳定文档名、章节名、能力名或验证项;不要让 `TODO-Px-xx` 成为长期架构锚点。
5. 每个 TODO 或主题形成一个独立提交,提交说明写清关闭的 TODO 编号和迁入位置。
6. 只有当剩余 TODO 都已有稳定归口且 `TODO.md` 不再提供额外判断价值时,才可以清空活跃区;清空后必须在 `TODO.md` 说明当前没有活跃项,并在 `CHANGELOG.md` 记录清空原因。

不得清空 TODO 的情况:

- 只是把开放问题移到别处,但没有主权文档承接。
- 需要用户裁决,且用户尚未给出方向。
- 真实工程验证还没有 Vxx 门禁、触发点、关闭条件和失败回写位置。
- 旧 TODO 编号仍被当前文档当作行为依据。
...
- 可保留:当时的阶段目标、偏差、风险、用户裁决、重大迁移路径和复盘材料。
- 不可保留为活跃主权:当前 TODO、未来排期、可勾选执行清单、未验证假设和变更流水线。
- 如果 progress 中的旧计划被新裁决取代,应把文件标为历史草案/废止,或在无当前链接依赖时删除;无论哪种做法,都要同步 `progress/README.md`。
- 实现阶段的真实验证证据归 `spec/appendix/Vxx`;跨文档收口归 `CHANGELOG.md`;开放项归 `TODO.md`。

## Agent 并行执行约定

多文档批量任务交给多个 coding agent 执行时,默认使用 multi subagents + git worktree 工作法:
...
- 如果真实工程验证需要可运行应用、fixture、外部服务或 spike harness,文档阶段只记录验证归口和关闭条件;实际通过/失败结果等实现时再回写 `Vxx` 与 CHANGELOG。
