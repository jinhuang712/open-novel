...
多文档批量任务交给 coding agent 执行时:

- 必须先按互不重叠的文件集切分任务,再**并行派发多个 subagent**;只派一个 subagent 串行执行不如不派。
- 同一工作树并行时,subagent/子代理只改文件或报告结果,不自行提交;主会话/整合者统一验证、写 CHANGELOG 并按主题提交。
- 共享文件(README、CHANGELOG、索引类文档)只分配给一个 agent,或由主会话收口处理,避免并发改写冲突。
