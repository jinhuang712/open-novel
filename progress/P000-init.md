...
> **归档提示:** 本文是 2026-04-29 启动期快照,包含旧框架、旧编号、旧排期和当时的技术假设。当前路线以 `README.md`、`plan/`、`spec/`、`TODO.md` 与 `CHANGELOG.md` 为准,不要把本文当作当前执行计划。

**日期**: 2026-04-29 **周次**: W1 **主要 Owner**: `jin.huang@klook.com`
...
- `spec/S01-storage-schema.md` — SQLite schema 与 frontmatter 规范
- `spec/S02-agent-tools.md` — Agent 工具签名与契约
- `spec/S03-prompts.md` — Agent prompt 模板
- `spec/S04-streaming-protocol.md` — SSE 事件协议
- `spec/S05-entity-highlight.md` — 实体高亮与跳转
- `spec/S06-approval-flow.md` — 审批流接入 needsApproval
- `spec/S07-mode-state-machine.md` — XState 状态机
- `spec/S08-de-ai-pipeline.md` — 去 AI 化 pipeline
...
- **Agent 数量**: 本日志记的 "6 Agent 拓扑" 在 progress/P002-narrative-reader (战略升级) 后扩到 7 (新增 ReaderPanel)。最新拓扑见 plan/01 §7 Agent 拓扑
- **时间表**: 原计划 ~12 周;002 升级后 ~14 周;004 docs 加固未变更代码工作量,仍按 14 周
- **W1 spec 集**: 当时仅 8 个 spec,后续 W2 docs 期扩到 13;004 docs 加固再扩到 16 (新增 spec/S00 spec/M09 spec/M08)
