...
**日期**: 2026-05-06
**周次**: 跨期 (W2 末尾, W3-W11 落地前)
**主要 Owner**: 自己
**Commit tree**: T1 → T2 → T3 → T4 → T5 → T6 (6 commits, 在 push 前 `c6d870e` 之后顺序提交;原拟 T7 progress 与 T6 不变性总束合并入同一 commit)

## 本期来源 (用户 5 个反馈点)

005 (知识图谱) + cascade flow 修正后,用户阅读发现"上下文 / memory 工程动作完全没有"。在补的过程中,用户连续给了 5 个根本性的设计反馈,每个都让前一个版本的设计反转:

1. **DeepSeek V4 ctx 是 1M 不是 128K** — 训练数据假设错了 8 倍
...
- ❌ 6 agent × 7 用途的 token 预算分配表 (基于 128K 假设, 实际 1M)
...
| spec/20 | assembleContext 默认装 5 类守则数据; tokenBudget 上限 80K → 800K (1M ctx); truncated 应永远空 |
