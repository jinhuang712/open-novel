...
> **[info]** audit 发现:awaitingApproval 中接受新 USER_INPUT 没建模 — 用户能不能同时回到 ChatBox 发新指令?如果 push 进 currentTask,与 await 残余状态如何 LIFO?
...
`Esc` 在 awaitingApproval 状态有特殊语义: **不**关闭 ApprovalCard (那是审批 dialog 的"无操作关闭"),而是 emit CANCEL → 走 spec/06 §Turn 取消语义 (整 turn revert, 把当前 turn.status 标 'cancelled' 区别于普通 rejected)。

## Approval Queue 建模 (W9 整批审之后简化)

> **[info]** ⚠ **W9 升级 (见 spec/06 / spec/19)**: cascade 改为审批前内部递归 + 整批审,**单个 ChangeSet 一个 approval 行**,内部 1-3 级 cascade 全在同一 ApprovalCard 勾选 — 不再需要"cascade 各项独立 approval 行"的子项排队建模。
>
...
> **[info]** audit 发现:spec/01 有 approvals 表 (status=pending),spec/07 又有 runtime/session.json 中的 pendingApprovals 数组。两边写入时机若不同步,reload 后会出现表里 pending 但 session 里没的不一致。
...
async function rehydrateMachine(projectId: string): Promise<MachineState> {
  // 拉 in-flight user_turns 的 pending approvals (无 24h 过期, 见 spec/06 §Turn 取消语义)
  const inFlightTurns = await db.userTurns.findActive(projectId)             // status IN ('running','awaiting_approval')
  const pendingFromDb = await db.approvals.findByTurns(
    inFlightTurns.map(t => t.id),
    { status: 'pending' },
  )
  // session.json 仅缓存 mode / currentTurnId; approvals/user_turns 永远以 db 为准
  const session = await fs.readFile('runtime/session.json').catch(() => null)
  return {
    mode: session?.mode ?? 'discuss',
    currentTurnId: session?.currentTurnId ?? inFlightTurns[0]?.id ?? null,
...
AI SDK runner 触发的事件(tool-call / finish / error)通过适配器转成机器 events。状态机不关心底层是 `generateText`、`streamText` 还是 JSON helper,只消费统一事件:

```ts
const stream = await callTextAgent({ label: 'writer', messages, projectId, threadId })
