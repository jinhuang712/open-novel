...
- 新增 spec/M01-shortcuts.md (完整快捷键 registry + 上下文化绑定 + 持久化 + 冲突检测)
- 新增 spec/M04-settings.md (SettingsDialog 8 section 详细设计 + 全局/项目级分层)
- 顺手修正 plan/05 (Tab 切模式行为) / plan/07 (重指向新 spec) / spec/S07 (Tab 触发的状态机 transition)
...
| 文件 | 改动 |
|---|---|
| `spec/M01-shortcuts.md` | 新建 |
| `spec/M04-settings.md` | 新建 |
| `plan/05-modes-and-approval.md` | 加 Tab 切模式行为,await_approval 阻止规则 |
| `plan/07-ui-layout.md` | 把"快捷键"段改成"详见 spec/M01";"Settings Dialog"段重写为"详见 spec/M04" |
| `spec/S07-mode-state-machine.md` | 状态机加 `SWITCH_MODE_VIA_TAB` event,与现有 `SWITCH_MODE` 区分 (Tab 来源 vs UI 点击) |
| `README.md` | 文档导航追加 spec/M01 + spec/M04 |
...
- spec/M01 spec/M04 docs **当前 PR 落盘**
