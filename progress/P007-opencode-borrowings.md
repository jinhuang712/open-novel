# P007 — opencode 借鉴落地 + TODO closure
...
- **[x]** 关闭 plan / spec 内累积的"待决策"TODO (spec/18 / spec/20 / spec/21 / spec/S01 反馈表 schema) (T8)
...
| commit | 改动 | 关键洞见 |
|---|---|---|
| `5da548b` T1 | spec/S00 §C 模型守约 + §H/I/J 新增 3 项 audit | 禁 fallback v3/-reasoner; cache_control / Mastra middleware / embedding 都需 W3-W8 verify |
...
| `cd4ad4b` T4 | spec/S02 工具输出 truncate + 6 个 hidden 内部 agent | TOOL_OUTPUT_MAX_CHARS=2000; HIDDEN_AGENT_REGISTRY 统一封装 callJsonAgent |
| `91466ed` T5 | spec/24 callJsonAgent 用 wrapLanguageModel + deepseekMiddleware | DeepSeek 特殊性透明化, agent 调用不感知 |
| `56ae0bf` T6 | spec/S06 doom-loop 检测 | 同 chapter 连续 3 次 Writer 输出相似度 ≥ 0.9 → escalate; 借 opencode processor.ts:351-374 |
| `edc4296` T7 | spec/S03 prompt stable+dynamic; plan/04 session_history.db; spec/S01 反馈表 schema | 关注点分离: 产物=Markdown, 过程=SQLite |
...
5. **"plan/spec 留了一些我的 todo"** → T8 批量 closure: spec/18 / spec/20 / spec/21 / spec/S01 narrative_feedback / spec/S01 entity_match_feedback
...
### 短期 (W3 day-1 spec/S00 audit 跑实查)
...
- `5da548b` T1 — spec/S00 DeepSeek 模型守约 + 3 项新 audit
...
- `cd4ad4b` T4 — spec/S02 tool 输出 truncate + Hidden 内部 Agent 类别
- `91466ed` T5 — spec/24 callJsonAgent 用 wrapLanguageModel + deepseekMiddleware
- `56ae0bf` T6 — spec/S06 Validator-Writer doom-loop 检测
- `edc4296` T7 — spec/S03 prompt 拆段 + plan/04 SQLite session_history + spec/S01 反馈表 schema
