# P006 — 记忆 / 上下文 / JSON 输出 / 五大守则: 一致性优先重设计
...
1. **DeepSeek V4 ctx 实查 1M tokens** (spec/S00 §C) — 早期估算偏低,设计骨架必须按 1M 重定位
...
- spec/S00 §C 状态从"待实查"改为"已实查 ✓ 2026-05-06":model ID `deepseek-v4-pro` / `deepseek-v4-flash`,ctx **1M**, max output **384K**
- spec/S00 §G (新增): JSON 输出模式审计 — `response_format: { type: 'json_object' }`,不强 schema (要应用层 zod),system/user 必含 'json' 字样,max_tokens 没设好会中途截断 JSON
...
- 与 spec/S03 / spec/S04 / spec/M09 的引用
...
| spec/S02 | 工具内部 LLM 调用规约: extractSemanticDelta / filterByLLM / concept extractor / compress 必经 callJsonAgent |
| spec/S03 | 公共片段 8 输出形态; Writer prompt 加 5 守则 + 黄金三章特殊版 + 反例 |
| spec/S04 | 4 个新 chunk 类型 (analyzing / json-result / json-retry / json-failed) + JSON streaming 衔接代码 |
| spec/S06 | ApprovalCard 加 CardinalRulesReportPanel + critical 必勾 + blocking 禁用 |
| spec/S10 | BeatAnalyzer + ArcTracker 都走 callJsonAgent + 加 cardinal contributions |
| spec/S11 | personaReactionSchema 加 dropoffRisk + cardinalRulesFlags; report 加 cardinalRulesFindings |
| spec/M11 | character.md zod v3; foreshadowings metadata deadline / weight / pattern |
