...
- [ ] H1 + 定位段:面向番茄小说作者的 AI 长篇创作工作台;像 IDE 一样工作;"驾驶位上的创作合伙人,不是一键生成器"。
...
- [ ] 七条原则,每条固定四段(**原则 / 含义 / 约束什么 / 反模式**):
  ...
  2. **透明非黑盒** — AI 团队在想什么、查了什么全程可见 → 反模式:不可见的内部决策。
  3. **给信号不替决策** — 诊断与预演只标记不阻断 → 反模式:总分排行 / gamification、强制修正、低分阻断发布。
  4. **心流保护** — 一切诊断在章节完成后批量给出 → 反模式:写作中途实时打断。
...
- [ ] `## 透明工作台`:每个角色的推理过程与动作实时可见、可回看、可复制粘贴给 AI 当反馈。
