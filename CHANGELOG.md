...
| 变更 | 影响文档 | 关联 |
|---|---|---|
| Onboarding 改为先选择/创建作品,空白作品只填作品名;流派、风格和故事种子进入工作台后由讨论模式承接。连接创作服务移动到进入正式工作台前最后一步,测试通过前不能跳过或进入工作台。 | `README` `spec/M15` `design/05` `design/prototypes/05-onboarding.html` | 用户反馈连接创作服务应为入台前强卡,启动页不应询问故事种子和类别。 |

## 2026-06-13 · docs/design · 设置分区去重

| 变更 | 影响文档 | 关联 |
|---|---|---|
| Settings 删除独立「帮助」分区;连接凭据分区只保留凭据录入、显隐、测试连接和失败说明,不再展示本月用量指标;用量指标独立留在「用量」分区只读展示。 | `spec/M14` `design/04` `design/README` `design/prototypes/04-settings.html` | 用户反馈「用量和连接凭据好像重复」「不需要帮助」。 |

## 2026-06-13 · docs/todo · TODO 空态措辞收敛

| 变更 | 影响文档 | 关联 |
|---|---|---|
| TODO 已清空后,同步 README 与 WORKFLOW 中的入口措辞:TODO 只保留需要裁决、无主权归口或设计/文档不同步的问题;实施期真实验证归 V01/V03,不再称 TODO 为活跃验证清单。 | `README.md` `TODO.md` `WORKFLOW.md` | 全局自查发现的文档口径漂移。 |
...
| 变更 | 影响文档 | 关联 |
|---|---|---|
| 裁决 Discuss 默认不学习:普通讨论、方案比较、过程追问和临时偏好只作为本轮材料;用户显式说“记住这个 / 以后都这样 / 把这条当规则”时才生成可见经验候选,并继续走 Reflector 去重、冲突和确认流程。 | `spec/M04` `spec/S01` `spec/S03` `spec/M12` `V01` `TODO.md` | 关闭 TODO-P1-47。 |
...
| 变更 | 影响文档 | 关联 |
|---|---|---|
| 新增 TODO-P0-01:要求为界面交互建立 branch matrix,把 pending、错误、空态、降级、确认、取消、恢复、只读锁定、焦点返回、移动端等关键分支逐一渲染为可打开 HTML 原型或同页状态;同时明确作者界面不得暴露 `.md`、相对路径、文件夹结构或任何存储实现细节,截图中的路径动作需改为作品对象动作。 | `TODO.md` | 用户追加要求“界面交互方方面面所有分支都渲染一遍 html”,并指出“打开 items/luopan.md”这类路径泄漏不应出现在界面。 |
