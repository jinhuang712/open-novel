...
> audit 发现: "工具调用若已开始则继续完成"过于乐观 — writeSetting 已写一半 (200KB 写到一半) 然后取消,fs 看到截断文件。需要更细粒度。
...
> audit 发现:`onFinish` 只在正常结束时写,timeout/error/abort 时 trace 不写。但用户最关心调试的恰好是错误。
...
> audit 发现:ReaderPanel 5 persona 并行 ≥10s,Validator 全项目扫 ≥30s — 没有进度信号,卡住时用户唯一操作是刷新 (回到刷新→cancelled 路径)。
