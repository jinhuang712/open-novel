...
| 审批后项目事实 | `project.db` | 已接受的 ChangeSet、写入记录、审批终态、obligation、文件指纹 ledger、持久 turn 状态 | 是 | 解释“这次变更何时生效”;损坏处理见 TODO-P1-60 |
...
所有会改变作者文件、项目事实库、审批终态、obligation、recap/activity 投影或恢复水位的动作,都必须留下不可倒改的写入记录。它的产品意义是回答三个问题:用户到底批准了什么、文件有没有真的替换、如果中途崩溃下次打开该怎么解释。

底层是否使用 append-only apply journal、记录哪些阶段、用户能看到哪些恢复文案,已进入 [TODO-P1-59](../TODO.md) 待讨论。本篇先保留最小事故收场:

| 阶段 | 写什么 | 崩溃后怎么收场 |
|---|---|---|
| prepared | 记录意图、前置条件、输入指纹、临时文件路径和恢复策略 | 启动扫描时若未 touching 原文件,标记该 prepared 写入作废;用户看到未生效。 |
...
跨文件 ChangeSet 不是文件系统原子事务。用户侧只能看到整批成功、整批失败、或需要内部恢复;系统内部可以前滚已替换文件并阻断后续写入,直到恢复到可解释状态。恢复结果也必须留下新记录,不能修改旧记录来假装事故没发生。

写入阶段不是 turn 终态。S14 只向 S03 返回写入记录阶段、已生效文件、项目事实提交状态、reindex request 和恢复建议;最终 turn 结果必须取自 [S03 · Canonical turn terminal enum](./S03-turn-orchestration.md#canonical-turn-terminal-enum)。`committed` 通常投影为 S03 `Applied`;无法完成提交但仍有恢复入口时投影为 `ApplyFailed`;自动恢复不安全时投影为 `ManualRecoveryOpened`;宿主或工具状态未知时投影为 `Interrupted`。S14 不另行定义 `success`、`done`、`abandoned` 或 `recovered` 作为 turn 终态。
...
| inline review accept | 生成轻量 accepted edit,不进入 cascade card | 原文范围、用户接受版本、提交前 undo bridge、风险重检结果。 |
| Humanizer 小改接受 | 生成轻量 accepted edit | diff、不可改事实校验、风格来源、reindex request。 |

Light apply 与审批 apply 共用 fencing token、fingerprint ledger、写入记录和 reindex/degraded 收场。差别只在用户交互重量:小改就地审定,大改整批审批。任何 light apply 若触及跨文件、设定、关系、伏笔、事实库治理或阻断级风险,必须升级为 ChangeSet 或 planning prerequisite,不能继续伪装成轻量保存。

Undo bridge 只存在于提交前。Inline review 或 Humanizer 小改被接受后,在保存或 light apply 提交前仍是编辑器缓冲区里的本地替换,可以被普通 editor undo 移除,也不会留下写入记录。保存成功、写入记录进入 `committed` 后,旧记录不可改写;用户撤销一个已提交 light apply 时,系统生成新的反向 light apply entry,向前追加,并重新触发 reindex/obligation 投影。
...
`project.db` 损坏时,审批历史、obligation、版本指纹和恢复记录可能无法从 Markdown 文件完整重建。是否需要把这种状态命名为 `facts-degraded`、用户看到什么、哪些写入要阻断、是否只有“以作者文件为准重建最小事实库”一条出路,已进入 [TODO-P1-60](../TODO.md) 待讨论。

本篇先保留两条底线:作者文件仍是可取用的正文事实;系统不能把派生索引重建结果伪装成丢失的审批历史或用户裁定。
...
| 项目事实库写失败 | 写入记录决定是否 file-applied | 落盘未完成,进入恢复 | 启动扫描前滚/放弃/人工处理 |
...
| 项目事实库真源损坏 | 作者文件仍优先 | 进入待讨论的显式恢复/保护路径 | 按 TODO-P1-60 裁决恢复方案 |

## 与其他 spec 的握手

| 对方 | Project Storage 给它什么 | Project Storage 从它要什么 |
|---|---|---|
| [S03](./S03-turn-orchestration.md) | 落盘结果、内部恢复结果、冲突状态和修正建议 | 已审批 ChangeSet 和事务意图 |
