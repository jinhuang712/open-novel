...
| 项目事实库 | `project.db` | 已接受的 ChangeSet、账本记录、审批终态、obligation、文件指纹 ledger、持久 turn 状态 | 是 | 解释“这次变更何时生效”;账本语义见 S15,损坏处理见 TODO-P1-60 |
...
`project.db` 损坏时,审批历史、obligation、版本指纹和 S15 账本记录可能无法从 Markdown 文件完整重建。是否需要把这种状态命名为 `facts-degraded`、用户看到什么、哪些写入要阻断、是否只有“以作者文件为准重建最小事实库”一条出路,已进入 [TODO-P1-60](../TODO.md) 待讨论。
