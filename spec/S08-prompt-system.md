...
```mermaid
flowchart LR
  Task[S04 task envelope] --> Prompt[S08 Prompt System]
  Context[S07 context package] --> Prompt
  ...
  Packet --> Runner[S03 Agent Runner]
  Packet --> Harness[S10 LLM Quality Harness]
...
| 导入资料 | fenced content + import source | 不能要求工具越权读取或写入。 |
...
| `untrusted_sources` | 标记正文、导入、检索、工具结果等围栏来源。 |
...
| context 注入顺序或摘要策略变化 | 不可以 | S07、S08、S10、S11。 |
