...
S07 拥有 context 侧预算装配,但不拥有价格表和审批批次决策。任何跨章节 cascade 在交给 S03 前,必须先形成 preflight budget,让 S04 决定是一次提交、分批执行、要求用户确认,还是因为证据不足/成本不可解释而失败。
...
| runtime estimate input | 输出模型调用次数、预计 reindex 范围、stream 心跳要求和可中断点 | I01 提供 provider 价格/上限,S04 生成用户级估算 |

如果 S07 无法给出可解释的 batch 和预算来源,不能把 cascade 包装成“正在处理全书”。它必须返回 budget gap 或要求缩小范围。
