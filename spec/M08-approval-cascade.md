...
| Mechanical Text Consistency | 称谓、旧名、直接引用、事实性短替换 | 是否随主事实保持一致 |
| Risk Notes | 守则冲突、设定冲突 | 是否需要人工改写 |
| Low Confidence | 锚点不稳、语义召回命中 | 默认不自动选中 |

低置信项不能混在普通连带修改里自动通过。它们必须显式降权,并允许用户打开来源判断。

机械一致性正文项只允许出现在 Planning cascade 的同批审批中,前提是改动可以由已审定事实直接推出,且不需要新的文学判断。重写场面、补情绪、改对白走向、调整节奏或重塑读者感受属于创作性正文修复,必须拆成 Writing proposal;Approval Cascade 可以把它们作为 residual obligation 或 Writing 前置项展示,但不能把它们包装成 Planning 的随批正文项。
...
pending approval 期间,Approval Cascade 是项目唯一可写焦点。Search、Quick Open、Trace 和 Discuss 可以只读查看或解释当前审批,但任何会生成新 proposal、创建新 ChangeSet、修改当前审批卡前置条件或切换到可写模式的命令都必须被禁用或提示先处理待审卡。
...
| 创作性拆分 | 场面、对白走向、情绪铺垫、节奏调整必须拆到 Writing proposal |
