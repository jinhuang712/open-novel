...
审批历史是项目可追溯性的一部分,默认随完整项目包导出。它解释每个 ChangeSet 的来源、分组、用户决定、落盘结果和失败收场,但不替代项目事实,也不把被拒内容重新变成可用正文。

| 历史内容 | 随项目包 | 说明 |
|---|---|---|
| ChangeSet identity / source / dependency group | 默认导出 | 用于追溯为什么产生本批修改 |
| 用户决定、拒绝理由、EditedAccepted 证据 | 默认导出 | 用于继续项目和审计责任 |
| applied item 与 reindex 状态 | 默认导出 | 用于解释哪些结果已生效、哪些索引过期 |
| pending / invalidated / residual obligation 摘要 | 默认导出 | 用于导入后继续处理 |
| rejected proposal 正文快照 | 默认不导出 | 只有用户在导出预览中显式选择才可带走 |
| Trace step / provider payload | 不属于审批历史 | 走 R05 诊断包并脱敏 |

导入项目包后,审批历史只能作为历史和待处理状态恢复。Applied 项不重新 apply;pending 卡必须重新校验来源锚点和项目事实水位后才可继续接受;invalidated 卡保持只读。缺失审批历史时,导入仍可保留项目事实,但必须标记“审批可追溯性不完整”。
...
| 审批历史导出失败 | 项目包标记审批追溯不完整或阻断完整导出 | 只导出正文却声称审批完整 |
| 导入后 pending 审批未通过重校验 | 卡片保持只读并提示重新分析 | 直接沿用旧包状态接受落盘 |
...
| 导出 | 审批历史默认随项目包,导入后 pending 需重校验 |
