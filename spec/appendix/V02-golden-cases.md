...
Golden Cases 保存 LLM、叙事诊断、ReaderPanel、Humanizer 和关键工作流的样例输入输出。它是验证明细,不是行为主权文档。[S10 · Evaluation And Golden Regression](../S10-evaluation-and-golden-regression.md) 定义 golden 如何成为质量门禁。
...
| ReaderPanel / S11 Creative Engine | persona 分歧、inconclusive、确认级/阻断级风险 |
...
| Long-form ability gate | 影响分析召回/精确、分段 delta 稳定性、cascade 体量/延迟 preflight |
...
| 全书级 cascade | 用量、耗时、失败恢复、分批审批和不能一次完成时的用户解释 | S07/S12 降级语义,后续用量契约 |

这些 fixture 没有原始 spike 证据时,对应 golden 状态应是 `needs data`,不能标记 pass。

## 边界

golden 可以帮助回归,但不能替代根层 spec。若 golden 与 spec 冲突,以 spec 为准并更新 golden;若 prompt/context/tool/runner 改动没有可判断的 golden 或 replay evidence,按 [S10](../S10-evaluation-and-golden-regression.md) 的 `needs data` 处理。
