...
每项目数据库物理拆分为真源库 `project.db` 和派生索引 `index.db`(见 [S14](../S14-project-storage.md)、[S15](../S15-journal-and-ledger.md)、[S01](../S01-runtime-state.md))。本节是字段族 → 数据库文件的唯一归属表;实现新表时必须先在这里落位,不得把真源字段写进 `index.db`,也不得让 `project.db` 出现可整体重建的派生对象。
