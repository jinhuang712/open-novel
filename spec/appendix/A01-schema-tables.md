...
- approval、ChangeSet、decision ledger、write/apply record、internal recovery、trace 的持久化字段。
...
每项目数据库物理拆分为真源库 `project.db` 和派生索引 `index.db`(见 [S14](../S14-project-storage.md)、[S15](../S15-journal-and-ledger.md)、[S16](../S16-file-version-and-edit-safety.md)、[S01](../S01-runtime-state.md))。本节是字段族 → 数据库文件的唯一归属表;实现新表时必须先在这里落位,不得把真源字段写进 `index.db`,也不得让 `project.db` 出现可整体重建的派生对象。
...
| project.db(每项目真源) | S15 ledger entry、decision record、write/apply record、recovery record、correction record、projection record、light apply transaction、persistent turn state、mode gate、approval queue、ChangeSet / approval item / decision、residual obligation、internal recovery snapshot、recap/activity 投影、author note / continuation action、host fencing、file version baseline、write echo token、edit safety state、revalidation result、protected ledger status、migration version、项目级经验(identity / content / state / reflector / audit)、danger action audit | 项目事实库损坏或不可信时进入 S16 保护状态;只能从当前作者文件重建最小可验证记录,不能用派生索引伪恢复审批历史 |
...
| Trace / Recap / Activity | trace step、tool run reference、S15 projection record、activity recap、author note、continuation action |
...
| 字段族 | 最少覆盖 | 行为约束 |
|---|---|---|
| file version baseline | project id、file id/path、content hash、anchor、mtime/size 辅助值、created for suggestion/approval/apply、watcher watermark | 每个未决建议、审批项和写入意图都必须绑定生成时看到的文件版本;mtime/size 不能单独证明文件未变。 |
| write echo token | write id、fencing token、file id/path、post-write hash、watcher watermark、expires/consumed state | 只有 token、文件身份、写后指纹和水位同时匹配,watcher 事件才可判定为系统自写回声。 |
...
| edit safety state | file id/path、state(clean/stale/invalidated/needs-revalidation/rebased/blocked-protected)、reason、affected pending ids、latest baseline | 这是 S16 的可写安全投影;不能当作 turn 终态或索引健康度。 |
| revalidation result | target id、old baseline、new baseline、result(still-valid/needs-rebase/regenerate/abandon)、evidence summary、user action | 重新校验只保留候选是否仍可信,不自动接受旧建议。 |
| protected ledger status | project id、status、known files、current content hashes、missing fact families、minimal rebuild audit、user-facing recovery note | 项目记录损坏或不可信时进入保护状态;高风险写入阻断,丢失历史不能由模型或索引补写。 |
...
## Turn / Ledger / Knowledge 字段族

S15/S14/S03/S05/S06 新增的主权对象按字段族归口如下:

| 字段族 | 最少覆盖 | 行为约束 |
|---|---|---|
| ledger entry | ledger entry id、project id、turn id、source kind、actor kind、created at、append-only sequence、supersedes/corrects pointer | S15 账本顶层身份;旧记录不可倒改,更正和恢复只能追加新记录。 |
| decision record | approval id、cancel plan id、danger action id、decision kind、scope、rationale、precondition snapshot、decision audit | 用户裁定、拒绝、取消、编辑后接受和危险操作确认的事实源;AI 不能伪造成用户裁定。 |
| write/apply record | turn id、action id、apply id、ledger entry id、stage(`prepared`/`file_applied`/`facts_committed`/`projection_committed`/`reindex_requested`)、fencing token、input/output file fingerprint、fact watermark、reindex watermark、recovery strategy、projection status | S14 只推进物理阶段;S15 解释阶段、恢复和 Recap/Activity 投影。阶段不是 S03 turn 终态。 |
| recovery record | recovery id、ledger entry id、recovery kind、known facts、unsafe reason、available actions、user recovery decision、resolved at | 崩溃、中断和 apply failure 后的恢复事实;不得自动重放危险动作。 |
| correction record | correction id、target ledger entry id、correction kind、new proposal/change source、approval id、result projection | 撤销、恢复到历史内容和回执更正都向前追加,不能修改旧记录。 |
| projection record | projection id、ledger entry id、projection kind、recap id/activity id/recovery note id、visibility、source links、missing/degraded reason | Recap/Activity 的事实来源;前端事件流只能通知 ready,不能临时拼装历史。 |
| light apply transaction | editor action id、source kind、selection anchor、accepted text/diff hash、undo bridge id、risk recheck result、ledger entry id、reindex request | direct edit / inline accept 不走大审批卡,但共用 S15 账本、S14 fencing、指纹和 reindex。 |
