...
| Settings / Credentials | provider config reference、credential reference、verification status、migration status、redaction/export exclusion audit |
| platform/Ixx | provider capability cache、editor adapter state、watcher cursor、import/export manifest、desktop permission、shortcut registration |
...
| provider credential reference | provider id、credential reference、verification status、last failure category、migration status、deleted at、export exclusion audit | secret 明文、token、secret 摘要和 keychain item path 不进项目库;只保存不可反推引用和状态。 |
...
| project package export inventory | content families、experience/analysis inclusion、recap/activity inclusion、approval history inclusion、trace attachment flag、credential excluded flag | 完整项目包默认带经验/分析、recap 和审批历史;凭据永远剔除。 |
