...
> 网文创作不能踩的五大坑,用户总结。本 spec 把 5 条守则**可机器化**:每条拆成可观察指标 + zod schema + 默认阈值 + 检测器伪码 + 风险报告输出格式。Validator / ReaderPanel / ArcTracker / BeatAnalyzer 各取自己擅长的维度并行扫,结果汇成 `risk_report` 进 ApprovalCard。
...
> 在黄金三章里反复横跳。开篇前三章基本决定了作品的生死。

> 症状:开篇不写主角,大量笔墨描写天气、街景、世界观宏大设定;或主角出场就窝囊废柴好几万字;"开局开会"群角色聊阴谋读者一脸懵。
...
> 给了读者一个无法兑现的承诺。"战神归来发现女儿住狗窝"是经典反面教材。
...
ALTER TABLE foreshadowings ADD COLUMN deadline_word_count INTEGER;
ALTER TABLE foreshadowings ADD COLUMN weight TEXT NOT NULL DEFAULT 'minor'  -- 'critical' | 'major' | 'minor'
  CHECK (weight IN ('critical', 'major', 'minor'));
ALTER TABLE foreshadowings ADD COLUMN expected_resolution_pattern TEXT;
```
...
async function promiseAccountabilityCheck(currentChapter: Chapter, config: PromiseConfig) {
  const violations: Violation[] = []
  const activePromises = await db.foreshadowings.findActive(projectId)  // status='planted', not resolved
  ...
  for (const promise of activePromises) {
    ...
    if (
      ...
      const recentTouched = await db.foreshadowings.touchedInChapters(promise.id, [
...
> 主角变成只会点击"领取奖励"的工具人。好的故事永远是人的故事。
...
│                                                              │
│ 🔴 critical (1):                                             │
│   人设崩坏: "林溪" 违反 reader_promise '杀伐果断'              │
│      段 ch_023#§5 表现为"瞻前顾后地不忍下手", value_axis(对敌) │
│      实际 0.32 vs baseline 0.85, 偏离 0.53 (阈值 0.4)        │
│                                                              │
│ 🟡 major (2):                                                │
│   节奏崩盘: 距上次 milestone (第 18 章"突破金丹") 已 5 章无新 │
│      milestone (阈值 5);若本章不推进进度建议加 milestone      │
│   期待感: critical promise "三年之约" 距 deadline 仅 7 章, │
│      最近 5 章未触及该承诺                                   │
│                                                              │
│ 🟠 warn (1):                                                 │
│   金手指: 滚动 10 章系统奖励段比例 32% (阈值 30%)             │
│                                                              │
│ ────────────────────────────────────────────                 │
│ ☐ 我已阅读上述风险,明知违反仍通过 (critical 必勾)             │
│ ────────────────────────────────────────────                 │
│ [拒绝 + 反馈] [同意章节 (Y)]                                  │
└──────────────────────────────────────────────────────────────┘
```
