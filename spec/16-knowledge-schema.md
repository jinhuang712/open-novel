...
> 实现 plan/11-knowledge-graph.md L1 数据层。本文档定义 4 张新表 + 1 张段锚表 (实现见 spec/17) + 1 张段嵌入表 (实现见 spec/18) + character.md frontmatter 升级 + 概念抽取流程。
...
CREATE TABLE entity_relations (
  ...
  UNIQUE(source_id, target_id, kind, since_chapter)   -- 防重复 reindex
);
...
CREATE TABLE entity_timeline (
  ...
  attribute TEXT NOT NULL,                -- 见 attribute 枚举
...
```
worldview/rules.md 落盘
   ↓
reindex Worker 入队 concept-extraction job
   ↓
LLM Flash 跑 spec/16 §concept-extractor prompt:
   "从这段文字中提取所有'世界硬规则'(影响后续创作的硬性约束):
    每条输出 { title, surface_forms, semantic, category, description }"
   ↓
返回候选 concepts (status='reviewing')
   ↓
SettingsDialog → 概念审视面板 (新)
  | 候选 | 类别 | 语义 | 表面词 | [接受] [拒绝] [改] |
   ↓
用户接受 → status='active' + 入 concepts 表 + concept_refs 全文重扫
用户拒绝 → status='archived' (留底防重复抽)
```
...
CREATE TABLE concept_refs (
  ...
  is_violation INTEGER NOT NULL DEFAULT 0, -- 1 = 与 concept.semantic 冲突 (e.g. concept='no phone' 但章节出现'手机')
  created_at TEXT NOT NULL,
...
CREATE TABLE dependencies (
  ...
  status TEXT NOT NULL DEFAULT 'pending', -- 见 status 枚举
  metadata TEXT,                          -- JSON: 收割章节 / 强度 / Custom 字段
...
CREATE TABLE paragraph_anchors (
  ...
  content_hash TEXT NOT NULL,             -- sha256(段内容)
  prev_anchor TEXT,                       -- 邻接段 (用于稳定性追踪)
  next_anchor TEXT,
  created_at TEXT NOT NULL,
  updated_at TEXT NOT NULL
);
CREATE INDEX idx_anchors_file ON paragraph_anchors(file_path);
...
CREATE TABLE paragraph_embeddings (
  ...
  embedding BLOB NOT NULL,                -- F32 array,长度 = embedding model dim
  model_name TEXT NOT NULL,               -- 'bge-m3' / 'deepseek-embedding-v1' / ...
  model_dim INTEGER NOT NULL,             -- 1024 / 1536
...
-- 是否建 vss 虚拟表 (LibSQL 原生 vector / sqlite-vss),见 spec/18 §选型决策
```
...
snapshot_dir 内容:
```
.snapshots/{projectId}/{ts}/
├── settings/                   # 完整 settings/ 拷贝
└── manifest.json               # { changed_files, agent, reason }
```
...
export async function assertNotDerived(filePath: string) {
  ...
  if (fm.derived === true) {
    throw new ToolValidationError('DERIVED_FILE_READONLY', `${filePath} 是派生文件,改动 SQLite 表即可`)
  }
}
```

writeSetting 调用前先 assert。读取派生文件正常(供 LLM / Writer 取用)。
UI: 打开派生文件时编辑器 read-only + 顶部 banner "派生视图,改动 SQLite 表后会自动重生成"。
