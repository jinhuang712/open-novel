...
这篇回答一个看似简单的问题:系统到底“记住”了什么?答案分三类:对话连续性、长期经验、过程证据。它们都叫运行时状态,但只有少数会影响后续生成,没有任何一类能取代作品事实。
...
|---|---|---|---|
| runtime.db | thread、message、压缩摘要、跨项目会话恢复状态 | 可以影响对话连续性 | 不能 |
...
过程历史再完整也只是证据,不是作品。会话消息再像“设定”,也不能绕过项目事实层。
...
| 用户经验 | 任务类型匹配、权重有效、未被关闭 | 项目事实和当前指令 |
| 过程 trace | 用户问“刚才为什么这样做”或 Debug 展示 | 不参与作品事实 |
| 失败记录 | 用于恢复、提示、诊断 | 持久 turn 状态 |

Agent 不能自己挑 runtime 材料。所有注入都经 [07](./07-context-and-query.md) 的 context builder。
