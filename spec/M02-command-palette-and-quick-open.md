...
pending approval 期间,命令面板和 Quick Open 只保留只读能力:搜索、打开、跳转、查看 Trace、打开当前审批卡和进入只读 Discuss。会生成新 proposal、创建 ChangeSet、接受跨文档改写、切换到可写模式或改变当前审批前置条件的命令必须禁用并说明“先处理待审审批”。
...
| pending 锁 | 待审期间只读打开/搜索/Trace 可用,新 proposal 命令禁用 |
