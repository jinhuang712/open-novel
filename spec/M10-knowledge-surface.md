...
```mermaid
flowchart LR
  Files[S01 Files] --> KG[S05 Knowledge Graph]
...
Pending approval 中尚未生效的事实不进入 Knowledge Surface 的派生展示。高亮、旁注、角色卡、backlinks 和 obligation 清单只能展示已生效项目事实、明确 pending 提示或 invalidated 历史;未审定 proposal 不得被当成当前世界事实。

单条展示必须带 freshness/staleness。索引 stale 时,高亮和引用可以保留但要弱化;锚点 missing 时禁止跳错段落;pending 时只显示“待审可能影响此处”的入口。用户看到的每条知识面板内容都要能追溯到来源或说明为什么来源不可用。
...
主界面旁注和高亮见 [design/01](../design/01-main-layout.md)。图谱管线见 [S05](./S05-knowledge-graph.md)。
