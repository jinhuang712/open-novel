...
>
> 拆分理由 (为什么 ArcTracker 不归 Checker) 见 plan/09-narrative-and-reader.md §叙事力学诊断(章内归 Checker、跨章归 Validator)。
...
两段视觉上同区呈现 (一个"叙事报告"卡),但数据来源是两个 Agent 的独立输出 (`CheckerReport.beats` / `ValidatorReport.arcs`,见 plan/09-narrative-and-reader.md §叙事力学诊断)。
