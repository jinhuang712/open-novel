...
开发构建中的测试者发现某个 turn 输出异常。打开 Developer Mode 后,他能看到该 turn 的全量 Trace 层级、派生文件、索引健康度和审计结果——全部只读。如果诊断指向索引损坏,Developer Mode 不提供「一键修复」按钮,而是把用户引导到 [R04 · Index Health And Repair](./platform/R04-index-health-and-repair.md) 的修复入口;修复属于危险操作,必须有独立确认和可见的失败收场。诊断包的生成与排障流程归 [R05 · Diagnostics And Debug Mode](./platform/R05-diagnostics-and-debug-mode.md)。

Developer Mode 受 dev build gate 控制。真实用户包不显示 Developer Mode,也不能通过 Settings、快捷键、URL、配置文件或本地存储开启;是否编入、是否暴露入口,由构建/打包变量决定。

```mermaid
flowchart LR
  Dev[Developer Mode] -.read.-> Runtime[S02 Runtime]
  Dev -.read.-> Storage[S14 Storage]
...
| dev build gate | 真实用户包不编入或不暴露 Developer Mode,运行时配置不能绕过构建门禁 |
...
入口只在 dev build 的 Settings Developer 区(见 [M14 · Settings](./M14-settings.md))出现。真实用户包没有该区,也没有可开启 toggle。dev build 开启后只影响只读诊断可见层级,不改变任何写入语义。
...
| 真实用户包请求 Developer Mode | 显示无入口或不可用 | 动态打开隐藏面板 |
...
| 遮蔽 | 诊断视图无 secret 明文 |
| 构建门禁 | 真实用户包无 Developer 入口,dev build 才可见 |
