...
| 等级 | 来源 | 能影响什么 | 风险与约束 |
|---|---|---|---|
| 1 | human edits / 用户当前指令 | 本次表达取向、明确改写偏好、作者亲手改过的措辞 | 最高优先级;只要不触碰事实和守则,后续风格判断必须服从。 |
| 2 | canon text / 已定稿正文 | 本书稳定叙述口吻、角色语感、节奏密度 | 只从已生效作品归纳,不能把 pending proposal 或未采纳建议当 canon。 |
| 3 | approved style / 用户批准的风格样本与 Settings 偏好 | 默认表达规则、禁用词、偏好的句式和节奏 | 可作为默认约束,但不能覆盖当前指令、正文事实或角色语境。 |
| 4 | AI suggestions / Reflector 经验与历史未审建议 | 候选提示、低置信参考、可解释的风格假设 | 只能作为建议来源;必须可见、可调、可删,不得自我引用成“作者风格”。 |

风格是表达约束,不是事实来源。

Humanizer 必须防止 AI 回声室:AI 生成内容不能因为被多次摘要、反思或建议而提高可信度。只有用户采纳、用户编辑后保留,或进入已生效正文的表达,才会提升为更高等级来源。来源冲突时按上表从高到低裁决;等级 4 与等级 1/2 冲突时直接降级或丢弃。
