...
批阅层未决标记有独立生命周期,不能只靠 DOM 状态存在。句内/段落建议在用户接受、拒绝、重试、升级为 cascade、来源失效或选区被外部编辑覆盖前都保持 pending;刷新、切换章节或重启项目后,仍应能恢复到“建议仍待处理”或“来源已失效”。接受后的标记进入 editor undo bridge;拒绝后保留最小记录供 Trace/Recap 解释,但不再作为待办提醒。
