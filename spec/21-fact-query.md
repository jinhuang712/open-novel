...
⏳ **Discuss 自由 LLM fallback 是否必须经 queryFacts?** — POC 阶段全部尝试解析 → 解析不成功才走自由 LLM。代价是每个 discuss 消息多一次 Flash 调用(~50ms);如果用户接受就保持,二期可加规则前置匹配降本

⏳ **UI 查询面板是否独立 modal vs 嵌入侧栏** — POC 用 Cmd+K → modal 形式快进快出;二期看用户使用率,高频可改右侧固定面板
