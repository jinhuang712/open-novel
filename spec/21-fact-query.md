...
async function tryParseAsFactQuery(message: string): Promise<{ success: boolean; query?: any }> {
  // POC 用 LLM Flash 短 prompt 分类 + 抽参数 (200 token 内)
...
- 实体下拉走 `db.entities.search` (POC 用 LIKE,二期换为 fuzzy)
- 章节下拉自动列出该项目所有章节
...
backlinks 面板("被 N 处引用")已有,可重构为 queryFacts mentions-of 的特化 UI。POC 阶段不必改,二期统一。
...
✅ **Discuss 自由 LLM fallback 路径**: POC 锁 **全部尝试 queryFacts → 解析不成功才走自由 LLM**。代价 (每个 discuss 消息多一次 Flash ~50ms) 用户接受。**W11 评估**: 高频规则匹配模式 (e.g. "X 是谁" / "X 在哪" 等) 加规则前置匹配降本; 但 W11 之前不做。

✅ **UI 查询面板形态**: POC 锁 **Cmd+K → modal 快进快出**。理由 = discuss 是低频高价值操作, modal 不占编辑器空间; 右侧固定面板会侵占已经很挤的写作视口 (spec/12 §快捷键已为 Cmd+K 留位)。**W11 评估**: 若用户高频用 (单 session > 20 次), 提供"固定到右侧"开关。
