...
真正的业务结果来自 [S03](./S03-turn-orchestration.md)、[S14](./S14-project-storage.md) 和 [S15](./S15-journal-and-ledger.md)。事件流只是把这些结果展示出来。
...
| control | awaiting approval、cancel requested、S03 canonical terminal result、recap ready、retry available | 审批卡、按钮、状态切换、recap 提示 |
...
stateDiagram-v2
  ...
  Running --> Idle: S03 Completed/StoppedNoChange
  Running --> Idle: S03 Applied/Cancelled/Rejected
  AwaitingApproval --> Idle: S03 Rejected/Cancelled/Applied
  Running --> Error: S03 FailedTerminal/ApplyFailed
  AwaitingApproval --> Error: approval invalidated or S03 ApplyFailed
  Error --> Idle: dismissed after safe state
```

状态点不能从单个 progress event 推断最终结果。它读取 turn 状态,事件只触发刷新。

S04 不拥有 turn 终态枚举。UI 状态点只投影 [S03 · Canonical turn terminal enum](./S03-turn-orchestration.md#canonical-turn-terminal-enum):`Completed`、`StoppedNoChange`、`Cancelled`、`Rejected`、`Applied` 回到可解释的 Idle/Recap 状态;`ApplyFailed` 和 `FailedTerminal` 投影为 Error 或 NeedsDecision;`Interrupted` 与 `ManualRecoveryOpened` 投影为 NeedsDecision/recovery。任何 control event 若携带终态,字段值必须来自 S03,不能使用 `success`、`done`、`abandoned`、`errored` 等本地替代值。
...
| 事件乱序 | 临时排序,最终以持久状态校正 |
| stream 断开 | 进入恢复流程 |
| 执行宿主重启 | UI 进入恢复中,从持久 turn 状态和写入记录读取最后状态 |
| Trace 写入失败 | 展示诊断不完整 |
| awaiting approval 到达 | 停止“自动运行中”的文案 |
| cancel requested | 显示正在取消,等待 turn 最终状态 |
| S03 `Interrupted` | 显示可恢复/可重试入口和最后可信 step |
| S03 `ApplyFailed` | 保留审批卡、S15 写入账本和 S14 物理阶段,展示重试/恢复/关闭入口 |
| S03 `ManualRecoveryOpened` | 展示人工恢复入口、已知事实和禁止自动重放原因 |
| post-apply reindex failed | 显示“作品已保存,索引降级”,提供 repair 入口 |
| recap ready | 状态点展示一句回执,Trace / Activity 可打开完整 recap |

## Recap 展示协议

Recap 由 S03 canonical turn terminal result 触发,由 S15 账本记录投影生成,不是前端从事件流临时拼出来。事件流只通知 UI 有一条 recap ready 或 recovery note ready;UI 恢复时必须从持久状态读取 recap/note。
...
作者可见 UI 使用“回执、恢复、活动记录、备注、更正”等产品语言,不展示 journal、ledger、write phase 这类内部系统名。
