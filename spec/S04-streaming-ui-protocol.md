...
真正的业务结果来自 [S03](./S03-turn-orchestration.md) 和 [S14](./S14-project-storage.md)。事件流只是把这些结果展示出来。
...
| S03 `ApplyFailed` | 保留审批卡和写入记录状态,展示重试/恢复/关闭入口 |
...
Recap 由 S03 canonical turn terminal result 触发,不是前端从事件流临时拼出来。事件流只通知 UI 有一条 recap ready 或 recovery note ready;UI 恢复时必须从持久状态读取 recap/note。
