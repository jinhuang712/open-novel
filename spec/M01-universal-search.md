...
Search 永远绑定当前 active project context。项目切换 preflight、目标项目挂载或恢复状态未收口时,Search overlay 必须关闭或进入只读不可查询态,不能展示上一个项目的候选结果。样例项目的查询历史、preview cache 和事实答案也按样例 project id 分桶;真实项目 Search 不读取样例命中,除非用户先把样例内容复制成新的真实项目并进入该项目。
...
- 项目切换发生时关闭当前 Search overlay;目标项目挂载完成前不能复用旧项目输入、结果或 preview。
...
| 样例/真实项目混淆 | 标记样例上下文或要求先切项目 | 把样例事实混入真实项目排序 |
