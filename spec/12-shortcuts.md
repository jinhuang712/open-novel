...
> **[info]** 替代之前散落在 plan/07 末尾的快捷键列表。所有快捷键集中在一个 registry 中管理,带上下文区分、冲突检测、用户可重绑、持久化。
...
| `view.toggleDeveloperMode` | 切换 Developer Mode (FileTree 显示 `_` 前缀 / DebugConsole 默认展开 / ThinkingPanel 全文等, 见 spec/13 §Section 8) | `Cmd+Shift+D` | ✓ |
...
> **[info]** **IME 闸门必须放在 listener 最前面**,在 `inferContext` 之前。否则中文用户在 ChatBox 用拼音敲字 → IME 候选页按 Tab 翻页 → 我们错把它当模式切换 → IME 候选闪退 + 模式被切。这是中文场景头号 UX 灾难,需专门处理。
...
```mermaid
flowchart TB
  subgraph SC["快捷键 设置面板"]
    H["上下文 | 描述 | 当前绑定 | 默认 | 操作"]
    R1["chatbox | 切换 Agent 模式 | Tab | Tab | [改]"]
    R2["chatbox | 反向切换 | Shift+Tab | Shift+Tab"]
    R3["chatbox | 发送消息 | Cmd+Enter | Cmd+Enter"]
    R4["..."]
    W["冲突: editor.acceptSuggestion 与 chat.cycleMode<br/>都绑定 Tab — 但上下文不同,实际无冲突"]
    A["[重置全部] [导出] [导入]"]
    H --- R1 --- R2 --- R3 --- R4 --- W --- A
  end
```
...
注意: 模式切换在 await_approval 状态下被状态机阻止 (见 [spec/07-mode-state-machine.md](07-mode-state-machine.md))。这种情况下:
...
## 命令面板与 CommandRegistry

> **[info]** `Cmd+Shift+P` 命令面板若仅 fuzzy search 已注册的 shortcut,80% 命令 ("创建第一个项目"、"导出 zip"、"切到角色 X" 等无快捷键的功能) 找不到。VSCode palette 核心是**所有命令** (含没快捷键的) 都进得去。
...
flowchart TB
  subgraph CP["命令面板"]
    Q["搜索: 切换"]
    H1["切换 Agent 模式 (discuss/plan/write) — Tab"]
    H2["切换批注 — Cmd+/"]
    H3["切换 Console — Cmd+`"]
    H4["切换 dark mode — (无快捷键也能搜到)"]
    R["最近用过"]
    R1["创建项目 (5 分钟前)"]
    R2["导出当前项目"]
    Q --- H1 --- H2 --- H3 --- H4 --- R --- R1 --- R2
  end
```
...
> **[info]** 支持 @文件名 引用 (类似 Cursor):触发逻辑 / 选中后插入 / 服务端 prompt 注入 / 多文件呈现。
...
> **[info]** 用户写 → AI 改 → 用户编 → AI 改,Cmd+Z 跳哪里没语义。TipTap 默认每个 transaction 一步,**AI 改写一次 replaceRange 整段就是一步,与用户字符级 history 混在一起**,Cmd+Z 5 次会跳过 4 个 AI 操作。
...
**Tauri 桌面壳 (二期)**: 上述限制大部分解除。Web 模式下文档 (本表 + Settings UI 的快捷键面板) 必须告知用户哪些键不可用。

## Modal Focus Trap

> **[info]** audit 发现:ApprovalCard 是 modal,内部按 Tab 应循环按钮 (拒绝/同意/编辑后同意),而不是切模式。
...
- **不做 chord (多键序列,如 `Cmd+K Cmd+T`)** — 所有快捷键单组合
