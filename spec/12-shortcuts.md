...
> **IME 闸门必须放在 listener 最前面**,在 `inferContext` 之前。否则中文用户在 ChatBox 用拼音敲字 → IME 候选页按 Tab 翻页 → 我们错把它当模式切换 → IME 候选闪退 + 模式被切。这是中文场景头号 UX 灾难,需专门处理。
...
> `Cmd+Shift+P` 命令面板若仅 fuzzy search 已注册的 shortcut,80% 命令 ("创建第一个项目"、"导出 zip"、"切到角色 X" 等无快捷键的功能) 找不到。VSCode palette 核心是**所有命令** (含没快捷键的) 都进得去。
...
> 支持 @文件名 引用 (类似 Cursor):触发逻辑 / 选中后插入 / 服务端 prompt 注入 / 多文件呈现。
...
> 用户写 → AI 改 → 用户编 → AI 改,Cmd+Z 跳哪里没语义。TipTap 默认每个 transaction 一步,**AI 改写一次 replaceRange 整段就是一步,与用户字符级 history 混在一起**,Cmd+Z 5 次会跳过 4 个 AI 操作。
