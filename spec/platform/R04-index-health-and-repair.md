...
Index Health And Repair 定义派生索引的健康检查、重建和修复。R04 只修每项目派生索引库 `index.db`(可整库删除后全量重建);真源账本 `project.db` 损坏不归 R04,其保护状态由 [S16](../S16-file-version-and-edit-safety.md) 定义。索引坏了不等于作品坏了,但必须可见。
...
## pre-apply 与 post-apply

`blocked` 只阻断新的依赖索引提案、影响分析和审批应用前置。它不能倒退已经通过 [S14](../S14-project-storage.md) 写入记录确认的作者文件。

| 时机 | 处理 |
|---|---|
| pre-apply 索引 blocked | S04/M08 不允许进入 Applying;用户需 repair、缩小范围或改为人工处理。 |
| Applying 中发现前置索引失效 | apply 停在恢复/needs decision,不得继续写入未知范围。 |
| post-apply reindex 失败 | 作品事实已保存;索引进入 degraded/repair,查询、高亮和后续高风险 Agent 降级。 |
| repair 后仍 blocked | 保留作者文件事实,阻断依赖索引的能力,提供诊断导出和人工入口。 |

M08 可以展示“已保存但索引降级”的 ApplyFailed/Degraded 状态;这不是审批失败,也不是自动回滚理由。
...
R04 只修派生索引。项目事实库真源损坏时,它只能报告“项目记录需要重新校验”,并交给 S16 的 protected facts ledger 保护路径;不能用 reindex 伪造审批历史、版本指纹或 obligation 解决状态。
...
| 项目事实库损坏 | 说明项目记录需要重新校验,R04 只能重建派生索引 | 把派生重建冒充事实恢复 |
