...
LLM Provider Contract 定义 Open Novel 如何接入模型服务。它不决定产品能力,而是证明模型边界真的支持 [S03 Agent Runtime](../S03-agent-runtime.md) 需要的调用形态。
...
flowchart LR
  ...
  Result -->|否| TODO[TODO / 路线调整]
  Contract --> Runtime[S03 Agent Runtime]
```
