...
flowchart LR
  ...
  Cursor --> Safety[S16 File Version Safety]
  Safety --> Storage[S14 Project Storage]
...
| fencing/reconcile signal | 宿主崩溃恢复、旧写入拒绝和启动对账的平台信号。 |
...
| 不匹配 write token 但指纹变化 | 投递 external edit,由 S16 判定审批/建议失效和 reindex 范围。 |
