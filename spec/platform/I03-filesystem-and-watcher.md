...
应用是单实例(见 [I05](./I05-desktop-shell-contract.md)):所有窗口都是同一宿主的 renderer 视图,不存在跨进程写入权竞争。

| 场景 | 平台行为 |
|---|---|
| 第二窗口打开同项目 | 以非可写窗口打开,展示当前可写窗口。 |
| 切换可写窗口 | 宿主进程内裁决:原可写窗口降级只读,新窗口获得写入权;切换是用户显式动作。 |
| 宿主崩溃后重启 | 新宿主持有新 fencing token,旧宿主残留的延迟写入被 storage 拒绝;先完成 journal 启动扫描和 reconcile,再开放高风险写入。 |
| pending approval | 写入权切换提示必须列出 pending 状态,不能静默关闭。 |

持久化的实例 id + fencing token 只用于崩溃防护:宿主异常退出后,新宿主启动时生成新 token,写入路径校验 token,任何携带旧 token 的延迟写入都必须被 storage 拒绝,并与 [S01](../S01-project-storage.md) 的 apply journal 启动扫描衔接。窗口间的写入权本身是宿主进程内状态,不依赖跨进程锁协议。
