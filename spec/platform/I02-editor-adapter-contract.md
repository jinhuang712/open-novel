...
| Replace Range | inline review 接受后替换文本;审批后批量改写由 storage/rollback 路径处理 |
| Undo Bridge | 区分本地 undo 和系统 rollback |
...
| undo 边界不明 | 禁止危险替换 | 混用 rollback 和 editor undo |
| IME 冲突 | 交给输入法 | 触发全局命令 |
...
**Q: 为什么 undo 和 rollback 要分开?**

A: undo 是编辑器局部编辑历史,rollback 是系统事务恢复。混用会让审批状态和文件状态不一致。
