...
这篇把 Agent Runner 写成一台受控执行器。Runner 只负责把一次已授权的 Agent run 跑完:调用模型、执行允许的工具、收集 step、校验结构化输出、在预算内重试,最后把结果交还给 [S03 · Turn Orchestration](./S03-turn-orchestration.md)。
...
```mermaid
sequenceDiagram
  participant O as S03 Turn Orchestration
  participant C as S06 Context Management
  participant P as S07 Prompt System
  participant B as S09 Tooling Boundary
  participant Host as Desktop Host
  participant R as S02 Agent Runner
...
Runner 在桌面壳的常驻执行宿主/sidecar 中运行,不绑定 renderer、普通浏览器标签页、开发服务器请求或单条 stream 连接。应用是单实例:一台机器同一时刻只有一个执行宿主,所有窗口共享同一个 Runner,窗口可写权切换不影响运行中的 run。UI 可以停止、恢复订阅、热更新或重新打开窗口,但这些动作不等于 run 成功、失败或取消;业务结果以持久 run envelope、step 证据和 S04 turn 状态为准。
...
| run 之间不共享可变状态 | 证据只能经各自 runner result 交回 S04;汇合语义由 [S03](./S03-turn-orchestration.md) 的质量汇合点定义,Runner 不做跨 run 合并。 |
...
| stream step | 把 step、tool call、partial text、failure 和 final result 映射为 [S04](./S04-streaming-ui-protocol.md) 可去重事件。 |
...
`interrupted` 是 Runner run state,不是一套独立 turn 终态。Runner 只能把 run 状态、最后可信 step、已完成 tool result、cancelability 和 checkpoint 交回 S03;是否形成 `Interrupted`、`FailedTerminal`、`ApplyFailed` 或 `ManualRecoveryOpened`,由 [S03 · Canonical turn terminal enum](./S03-turn-orchestration.md#canonical-turn-terminal-enum) 统一裁决。S04/S05 必须向用户展示“运行被中断、哪些结果可用、是否可安全重试”。只有幂等且没有 durable change 的 run 可以从 checkpoint 重试;已经进入写入记录的流程交给 S14 恢复,Runner 不得重放危险动作。

## Runner 不拥有什么

| 不归 Runner | 主权位置 |
|---|---|
| 哪些事实必须装入、哪些事实可裁 | [S06 · Context Management](./S06-context-management.md) |
| prompt 分层、优先级、不可信内容围栏 | [S07 · Prompt System](./S07-prompt-system.md) |
| 工具白名单、工具权限、二次 LLM 调用边界 | [S08 · Agent Tooling Boundary](./S08-agent-tooling-boundary.md) |
| canonical AI 角色 id、角色开关和用量归因 | [M13 · Agent Team Controls](./M13-agent-team-controls.md) |
| 真实任务回放、输入输出记录、失败复现 | [S09 · LLM Quality Harness](./S09-llm-quality-harness.md) |
| golden regression、质量指标、阻断门禁 | [S10 · Evaluation And Golden Regression](./S10-evaluation-and-golden-regression.md) |
| 审批、落盘、取消计划、recap | [S03 · Turn Orchestration](./S03-turn-orchestration.md) |
...
这里的 `Completed`、`Failed`、`Stopped`、`Interrupted` 都是单个 run 的结束状态,不等同 turn 终态。`Completed` 不代表作品已改变,只代表 Runner 得到一个可交给 S03/S04 的结果。`Stopped` 也不是失败;它会带上已完成 step 和不可用 artifact 的解释,由 S03 映射为 `StoppedNoChange`、`Cancelled` 或其他 canonical result。`Interrupted` 表示执行宿主或外部调用状态未知,只能交给 S03/S04 recovery 投影,不能自动生成成功结果。
...
| heartbeat/watchdog budget | S03 + host | 返回 run interrupted,由 S03 映射为 canonical turn terminal result,再由 S04 展示恢复入口。 |

每次重试都必须记录 `retry_reason`、`attempt_count`、`contract` 和最后一次失败摘要,供 [S09](./S09-llm-quality-harness.md) 复现。任何 retry budget 超限后,系统不能自动生成“再试一次”的新 turn。
...
- [A04 · Tool Catalog](./appendix/A04-tool-catalog.md) 保存工具参数明细;权限和失败语义由 [S08](./S08-agent-tooling-boundary.md) 定义。
- [A05 · Prompt Templates](./appendix/A05-prompt-templates.md) 保存 prompt 模板全文;prompt 主权由 [S07](./S07-prompt-system.md) 定义。
