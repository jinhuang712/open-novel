...
> 实现 plan/11-knowledge-graph.md L2 算法层第一项。本文档定义段锚点 (paragraph anchor) 的稳定 ID 算法、`paragraph_anchors` 表使用、差量 reindex 流程,以及段移动 / 重命名 / 拆分 / 合并的稳定性追踪策略。
...
## paragraph_anchors 表 (schema)

(已在 spec/16 §表 6 定义,此处补全语义)
...
CREATE TABLE paragraph_anchors (
  ...
  content_signature TEXT NOT NULL,        -- 归一化指纹 (anchor_id 用)
  prev_anchor TEXT,                       -- 前一段 anchor (双链)
  next_anchor TEXT,                       -- 后一段 anchor
  start_offset INTEGER NOT NULL,          -- 在 file 中的字符 offset
  end_offset INTEGER NOT NULL,
  created_at TEXT NOT NULL,
  updated_at TEXT NOT NULL,
  -- 软删除: anchor 失效但仍保留 (供 dependencies 标 broken),保留 30 天后清理
  deleted_at TEXT
);
...
```
writeSetting / writeChapter 落盘
   ↓
reindex Worker 取该 file
   ↓
读旧的 paragraph_anchors (WHERE file_path = X AND deleted_at IS NULL)
   ↓
按 splitParagraphs(新内容) 算新段集
   ↓
diff 算法 (见下) 判:
   - 哪些 anchor 不变 (未改)
   - 哪些 anchor 内容微调 (signature 同 但 content_hash 不同)
   - 哪些 anchor 删除 (旧的找不到对应)
   - 哪些 anchor 新增 (新内容没对应旧 anchor)
   - 哪些 anchor 是"重写" (旧 anchor 失效 + 邻接相同 + 新位置内容近似)
   ↓
对各类做对应处理 (见下)
   ↓
广播 'anchors:changed' 事件 → 下游 (entity_refs / concept_refs / embeddings / dependencies) 各自处理
```
