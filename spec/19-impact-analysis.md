...
> **[info]** 实现 plan/11-knowledge-graph.md L2 算法层第三项 + L3 工具层第一项。本文档定义影响半径计算器 (纯 SQL,不调 LLM)、`analyzeImpact` 工具签名、LLM 二次过滤流程、递归 cascade 终止条件。
...
```mermaid
flowchart TD
  W0([Writer agent 出主修改 in-memory]) --> V[Validator agent execute<br/>调 analyzeImpact 工具,传入 main delta]
  V --> LOOP["analyzeImpact 内部递归循环"]

  subgraph R1["第 1 轮 (round=1, weightFloor=30)"]
    direction TB
  ...
  LOOP --> R1
  R1 --> WS1["Writer 内部短调用<br/>对每条 needsChange=true proposal 生成 afterText (in-memory)"]

  subgraph R2["第 2 轮 (round=2, weightFloor=60)"]
    direction TB
    R2S1["step 1: extractSemanticDelta(round=1 afterText 集合)"]
    R2S2["step 2: computeImpactRadius — 必须严格收缩<br/>(候选 ⊆ 第 1 轮剩余)"]
    R2S3["step 3: filterByLLM, ChangeProposal[round=2]"]
    R2S1 --> R2S2 --> R2S3
  end

  WS1 --> R2
  R2 --> WS2[Writer 内部短调用 round=2 afterText]
  WS2 --> R3["第 3 轮 (round=3, weightFloor=90) 同上"]

  R3 --> TERM{"终止 — 任一成立即停:<br/>a. 候选段集为空<br/>b. 半径未严格收缩<br/>c. round = 3"}

  TERM --> SUM["汇总 ChangeSet:<br/>主修改 (cascadeLevel=0)<br/>第 1 轮 ChangeProposal (cascadeLevel=1)<br/>第 2 轮 ChangeProposal (cascadeLevel=2)<br/>第 3 轮 ChangeProposal (cascadeLevel=3)<br/>影响图谱 (节点 + 边,标 cascadeLevel)"]
  SUM --> RET[analyzeImpact 工具返回<br/>Validator agent 提交给 ApprovalCard]
  RET --> AC["ApprovalCard 一次呈现整批<br/>用户勾选 + 一键同意 / 部分同意 / 拒绝"]
  AC --> APR["用户 approve<br/>POST /api/approvals/{id}/resolve"]
  APR --> COMMIT[后端 transaction<br/>一次落盘所有 accepted 文件]
  COMMIT --> SIDE["落盘副作用<br/>差量 reindex / snapshot<br/>history group / Reflector"]
```
...
// 注册为 SQLite custom function (better-sqlite3 支持)
...
> **[info]** 走 [DeepSeek JSON mode (spec/24)](./24-json-output.md) — 用 `callJsonAgent` 统一 retry / zod 校验 / max_tokens 估算。

```ts
// lib/impact/llm-filter.ts
import { callJsonAgent } from '@/lib/agents/json-output'
import { ImpactFilterBatchOutputSchema } from '@/lib/agents/schemas'      // spec/24
...
export async function filterByLLM(
  ...
  const batches = chunk(candidates, 5)         // 每 5 段一批 (spec/24 §maxTokens 估算: 5 段 ≈ 2K)
  ...
  for (const batch of batches) {
    ...
    // Flash 模型 + JSON mode (二次过滤是二元判断, Flash 完全够用; Pro 留给 Validator 一致性审)
    const response = await callJsonAgent({
      label: 'impact-filter-batch',
      model: deepseekFlash,
      schema: ImpactFilterBatchOutputSchema,
      maxTokens: 2_000,
      messages: [
        { role: 'system', content: FILTER_SYSTEM_PROMPT },           // 见 spec/03 §filter 模板
        { role: 'user', content: prompt },
      ],
    })

    // 把 ImpactFilterBatchOutput.candidates 映射成 ChangeProposal[]
    proposals.push(...response.candidates.filter(c => c.affected).map(c => ({
      anchorId: c.anchorId,
      targetFile: anchorToFile(c.anchorId),
      needsChange: true,
      proposedText: undefined,                                       // afterText 由 Writer 内部短调用补
      reason: c.reason,
...
`ImpactFilterBatchOutputSchema` 见 [spec/24 §analyzeImpact LLM filter](./24-json-output.md)。
...
type AnalyzeImpactResult = {
  ...
  cardinalRulesReport?: CardinalRulesReport     // 五大守则风险报告 (spec/25);plan 模式 cascade 也会触发
  metadata: {
...
### Cascade 与五大守则的衔接 (spec/25)

cascade 不仅扫"事实一致性",还要扫"守则一致性"。两类敏感修改触发额外的全章节重扫:

| 修改类型 | 守则触发 | 额外扫描 |
|---|---|---|
| `character.md` 改 `reader_promises` / `taboos` / `value_axes` | 守则 2 (人设崩坏) | 全 active 章节里涉及该角色的段都要 Validator characterIntegrityCheck 重审 — 老 promise 已被改后,可能不再有"违反"问题,但**新 promise** 可能被既有章节违反 |
| `character.md` 改 `intelligence_axis` | 守则 2 (假智谋) | 全主角策略成功的章节 → 重审对手 IQ 是否合理 |
| `dependencies(kind='foreshadowing')` 的 promise metadata 改 `deadline_chapter` / `weight` | 守则 4 (期待感) | promiseAccountabilityCheck 全 active critical promise 重审 |
| `cardinal-rules.json` 阈值调整 | 守则 1/3/5 阈值 | 不重扫(阈值是"未来章节用",已审章节不重算;用户可点"重新评估当前章节"手动触发) |

实现:`writeSetting` 工具 execute 中,若发现修改命中以上类型,在内部递归 cascade 前**先**跑一遍守则相关的全 active 章节扫,把额外发现的 violation 进 ChangeProposal[round=1] 一起进 ApprovalCard。

**性能考虑**: 全章节扫只在守则相关字段改动时触发;普通"性别 / 名字"改动不触发(那些有自己的 cascade 路径)。
...
> **[info]** 注: 递归发生在 analyzeImpact 工具的**单次调用**内部,不是用户跨多个 ApprovalCard 的多次审批。用户只看到一次 ApprovalCard,内部已经迭代到稳态。

**流程图 · round = 1, weightFloor = 30**

```mermaid
flowchart TD
  IN([analyzeImpact 工具入口<br/>传入 main delta]) --> INIT["round = 1, weightFloor = 30"]
  INIT --> CIR["computeImpactRadius(weightFloor)<br/>出候选"]
  CIR --> CHK1{候选空?}
  CHK1 -->|是| END([break])
  CHK1 -->|否| FLT["filterByLLM(候选)<br/>ChangeProposal[round]"]
  FLT --> CHK2{"needsChange=true<br/>proposal 数 = 0?"}
  CHK2 -->|是 LLM 判全部不需改| END
  CHK2 -->|否| WSC["Writer 内部短调用<br/>各 proposal 生成 afterText (in-memory)"]
  WSC --> INC["round += 1"]
  INC --> CHK3{"round > 3?"}
  CHK3 -->|是| END
  CHK3 -->|否| WF["weightFloor += 30<br/>(30 / 60 / 90)"]
  WF --> NEXT["nextDelta = 把 afterText 集合<br/>算成 paragraph-rewrite + concept-* delta"]
  NEXT --> CHK4{"半径不严格收缩?<br/>(currSet 不是 prevSet 子集<br/>或 |currSet| ≥ |prevSet|)"}
  CHK4 -->|是| END
  CHK4 -->|否| LOOP["delta = nextDelta<br/>continue"]
  LOOP --> CIR
```
...
每次 analyzeImpact 调用记录到 `cascade_audits` 表。

> **[info]** **Schema 主权 (Wave 4)**: 完整 `CREATE TABLE cascade_audits` + INDEX 见 [spec/01 §cascade_audits](./01-storage-schema.md#cascade-audits)。

**字段摘要**: `project_id` · `triggered_by_approval_id` · `parent_audit_id` (递归层级, FK 自表) · `total_candidates` · `filtered_by_llm` · `proposals_count` · `llm_token_cost` (Pro tokens 累计) · `duration_ms` · `graph_json` (完整影响图)。
