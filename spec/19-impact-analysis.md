...
> 走 [DeepSeek JSON mode (spec/24)](./24-json-output.md) — 用 `callJsonAgent` 统一 retry / zod 校验 / max_tokens 估算。
...
| `foreshadowings` 表改 `deadline_chapter` / `weight` | 守则 4 (期待感) | promiseAccountabilityCheck 全 active critical promise 重审 |
