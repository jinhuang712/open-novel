...
Discuss Mode 是 Router action 中最严格的只读分支,详见 [13 · Discuss Mode](./13-discuss-mode.md)。它可以讨论、解释和建议,但不能生成可落盘 ChangeSet。
...
Approval Cascade 是 ChangeSet 的用户可见审定模块,详见 [15 · Approval Cascade](./15-approval-cascade.md)。本篇定义生命周期,15 定义审批 UI 必须解释什么。
...
- [appendix/json-schemas](./appendix/json-schemas.md) 保存 Router action、ChangeSet 和审批输出 schema。
