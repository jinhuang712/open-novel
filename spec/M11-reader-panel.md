...
ReaderPanel 是发布前读者预演能力。根层 [Creative Engine](./S11-creative-engine.md) 定义质量信号和守则;本篇定义读者报告如何生成、展示和进入审批解释。
...
| 入口 | 前置条件 | 结果 |
|---|---|---|
| 统一搜索命令候选 · 运行 ReaderPanel | 当前章节、选区或指定章节可读 | 进入 turn,生成新的读者风险报告 |
| 统一搜索命令候选 · 打开最近 ReaderPanel 报告 | 项目内存在最近报告 | 打开报告面板,不重新运行 |
...
[design/03 ReaderPanel](../design/03-reader-panel.md) 定义报告布局和可视化。行为主权以本篇和 [Creative Engine](./S11-creative-engine.md) 为准。
...
| 入口 | 统一搜索命令候选运行 / 打开最近报告 / 审批卡内嵌三条路径互不串语义 |
