...
| **不允许 agent 自拼 prompt** | 必经 context builder;直接裸调 `generateText` / `streamText` 是反模式 |
...
export async function buildBaseContext(input: BaseContextInput, agentName: string): Promise<{
  ...
  // 3. L2 应用层 memory messages (lastMessages 30, 见 spec/22)
  const recentMessages = await appMemory.fetchRecent({
    projectId: input.projectId,
    threadId: input.threadId,
    limit: settings.memory.lastMessages,
  })
...
**输出形态**: 自然语言流式 (不走 JSON mode) **maxTokens**: 不显式设(Writer 章节正文流式输出),由 AI SDK `streamText` 的 `finishReason` 判定自然结束或截断。
...
async function buildValidatorContext(input: ValidatorInput): Promise<BuildOutput> {
  ...
  const retrieved = {
    ...
    // 待处理伏笔全部 + 已 resolved 近 N 个;物理表是 dependencies, kind='foreshadowing'
    foreshadowingsAll: await db.dependencies.findForeshadowings({ projectId: input.projectId, status: { in: ['planted', 'resolved_recent_20'] } }),
    // 世界观全文
    worldviewFull: await readWorldviewFull(input.projectId),
    cardinalRulesConfig: await readCardinalRulesConfig(input.projectId),
  }
...
**输出形态**: JSON mode (见 spec/24 §Validator 一致性审 输出 schema) **maxTokens**: 16K (proposals 多时,见 spec/24)
...
async function buildReflectorContext(input: ReflectorInput): Promise<BuildOutput> {
  ...
  // 不装应用层 memory messages, 不装 L4 — 纯学习场景
...
[
  ...
  ...recentMessages,                                                // L2 应用层 memory 最近 N 条
...
> **[info]** ⚠ DeepSeek V4 真实 tokenizer 与 GPT-4 略有差异 (中文 token 化更紧),tiktoken 估算误差 ±5%。加 margin 够用,后续切换到 DeepSeek 官方 tokenizer (如提供)。
...
每次 context builder 调用,在 index.db 写一条 trace。

> **[info]** **Schema 主权 (Wave 4)**: 完整 `CREATE TABLE prompt_traces` 见 [spec/01 §prompt_traces](./01-storage-schema.md#prompt-traces)。

**字段摘要**: `thread_id` · `agent_name` · `mode` · `json_mode` (0/1) · `token_report` (JSON) · `metadata` (JSON: learningsApplied / retrievedKeys / jsonMode / etc.) · `duration_ms`。
...
const stream = await callTextAgent({
  label: 'writer.write',
  model: deepseekProMax,
  projectId,
  threadId: ctx.threadId,
  messages: ctx.messages,
  // system 已经在 messages[0] 了, 不再单独传 system 参数
})

await db.workspace(projectId).promptTraces.insert({
  id: ulid(),
...
**注意**:runner 不再自动追加历史消息。所有 L2 history 都由 context builder 显式调用 `appMemory.fetchRecent` 后放进 `messages`。这样可以避免框架层重复注入 lastMessages,也让 Router 只取最近 5 条、Writer 取 30 条等差异化策略可见可测。

## 应用层 lastMessages 读取边界

> **[info]** lastMessages 是 memory 模块的读取参数,不是 LLM provider 或 Agent runtime 的隐式行为。任何调用如果没有经过 context builder,就不会获得历史消息。

默认值为 30,Router 可在自己的 builder 中降到最近 5 条。测试需要覆盖每个 agent 的读取数量,防止历史上下文被静默扩大或缩小。
...
describe('per-agent context builders', () => {
  ...
  it('Reflector 不装应用层 memory messages (它是写入者)', () => { /* ... */ })
...
- **应用层 lastMessages 读取边界**: 每个 agent 的读取数量必须由 context builder 显式定义并测试
- **超长章节的 retrieve 实操**: 单章 50K+ 字时 readRecentChapters 5 章可能 250K+,context builder 不裁但要监控;若高频触发警报再单开专题
