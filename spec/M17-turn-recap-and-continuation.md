...
Recap 是用户回执,不是作品事实。后续生成仍以项目文件、S15 决策与写入账本中的审批/写入记录和 story world 为准。
...
flowchart TB
  ...
  Ledger["S15 Ledger<br/>裁定 / 写入 / 恢复"] --> Recap
...
| S15 Ledger | 系统 | 裁定、写入、恢复、反向修正、危险操作审计和投影来源 | append-only;旧记录不倒改 |
...
`AwaitingApproval` 不是终态。它可以通过 S15 生成 pending activity item,显示待审范围和恢复入口,但不能生成“已完成/已放弃”的 recap。纯本地打开、搜索、预览和跳转不是 turn。它们可以更新 session 内的最近项、查询缓存或 UI 事件,但不能生成 recap,也不能把项目 Activity 时间线刷成操作日志。
...
Recap 用用户语言,不暴露表名、工具参数、provider payload、内部 prompt 或 journal/ledger/write phase 等内部系统名。
...
反向修改和恢复提案本质上都会改变正文或设定,必须进入审批或 S15 定义的 light apply 入口。Recap 只能发起这些动作,不能绕过审批、账本和 S14 落盘协议直接改文件。
...
| 失败 | 用户看到 | 系统不能做 |
|---|---|---|
| recap 生成失败 | turn 结果仍可见,S15 标记 projection 缺失 | 阻断已审批落盘 |
| activity 写入失败 | 告知历史记录未保存,S15 保留可补写状态 | 假装已进入 changelog |
...
A: 默认不能当事实源。用户明确要求“根据上次 recap 继续”时,它可以作为参考材料,但与项目事实或 S15 写入账本冲突时必须让位。

**Q: 用户能不能删除一条 recap?**

A: 原始 recap 不删除、不改写。可以归档、筛选或隐藏某类记录,但 S15 账本保持 append-only。
...
- [S15 · 决策与写入账本](./S15-journal-and-ledger.md) 定义 Recap/Activity 的账本来源、恢复记录和反向修正边界。
