...
Recap 不只出现在失败或取消后。正常完成、等待审批、拒绝、放弃、停止、部分失败、已落盘、已修正,都生成一条项目级记录。

## Recap 的内容

| 区块 | 示例 | 必须性 |
|---|---|---|
| 状态 | 已完成 / 已停止 / 等待审批 / 已放弃 / 已修正 | 必须 |
...
Recap / Activity 是作者级项目历史,默认随完整项目包导出。它们帮助用户换设备或恢复项目后继续理解“刚才发生了什么”,但仍不是项目事实源。

| 内容 | 默认导出 | 导入后语义 |
|---|---|---|
| turn 状态、作品影响、可用结果、未完成项 | 是 | 作为历史时间线展示 |
| 下一步和 continuation action | 是 | 需要重新检查当前项目状态后才可执行 |
| 审批卡、章节、Trace 的来源跳转 | 是 | 跳转目标不存在时标记过期 |
| 作者备注和 correction recap | 是 | 保持 append-only |
| 分析摘要 / ReaderPanel 摘要 | 是 | 可作为历史结果查看,不是事实源 |
| Trace 详情、provider payload、内部 prompt | 否 | 只通过 R05 诊断包显式导出 |

导出预览必须说明 Recap 可能含有正文片段、拒绝理由或作者备注。用户选择排除敏感作者备注时,系统仍要保留 recap 的状态、时间和作品影响摘要,并标记备注已被用户排除。导入后如果 recap 与项目事实冲突,界面必须提示“历史记录与当前事实不一致”,后续生成仍以项目事实和审批记录为准。
...
如果已经生成 pending ChangeSet、开始落盘或存在不可自动恢复风险,取消入口必须交给 [S04](./S04-turn-orchestration.md) 的 cancel plan,由用户确认具体影响。
...
| Recap 状态 | 是否可学习 | 原因 |
|---|---|---|
| 已完成且有作者审定 | 可以 | 用户作出了明确决定 |
| 已停止 | 不可以 | 用户只是中止,不是偏好 |
| 已放弃 / 已拒绝 | 只可记录拒绝理由,不自动学成偏好 | 需要明确反馈才可学习 |
| 已修正 | 可从最终审定结果学习 | 修正动作本身经作者确认 |
...
| Recap 导出被用户排除备注 | 导出预览和导入后标记备注缺失 | 把删减后的历史伪装成完整原文 |
| 导入后 continuation 过期 | 提示重新运行或重新生成提案 | 直接执行旧 continuation |
