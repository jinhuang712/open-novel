...
BeatAnalyzer 不是第八个 canonical role id。它是 `checker` 角色内部的结构诊断工具或 prompt 片段,用于节奏、爽点、承诺推进和章内结构分析。事件、Trace、成本、设置开关和 prompt 名称仍归 `checker`;需要展示 BeatAnalyzer 时只能作为工具/步骤名,不能作为持久化角色值。
