...
以下 id 是持久化、事件、Trace、用量归因和 prompt 名称的唯一角色标识。中文名是用户可见展示,可以本地化;id 不能随文案变化。
...
这组 id 不是 runner 数量。Runner 执行契约由 [S03](./S03-agent-runner.md) 定义,同一个受控 Runner 可以按 run envelope 承载不同 role;文档不得用“13 个 runner”等未落主权清单的数字替代这里的角色枚举。

BeatAnalyzer 不是第八个 canonical role id。它是 `checker` 角色内部的结构诊断工具或 prompt 片段,用于节奏、爽点、承诺推进和章内结构分析。事件、Trace、用量、设置开关和 prompt 名称仍归 `checker`;需要展示 BeatAnalyzer 时只能作为工具/步骤名,不能作为持久化角色值。
