...
这篇把上下文管理写成一套“证据包供应链”。它负责把本次任务真正需要的项目事实、原文、锚点、用户经验和缺口说明组装成 context package,交给 [S02 · Agent Runner](./S02-agent-runner.md) 使用。
...
当来源文件版本处于 S16 的 `NeedsRevalidation`、`Invalidated` 或项目记录保护状态时,context package 不能把旧锚点和旧审批依据装成可信事实。它必须返回 context gap,说明文件已外部修改、需要重新校验,或把任务降级为只读解释。
...
卷/册边界来自作者文件和项目结构,不是模型临时猜测。作者显式建立卷、册、部、篇或等价层级时,S01 记录源结构;S06/S07 基于该结构生成 volume summary、book/part summary 和失效水位。若项目没有显式分卷,S07 可以建议分卷或按章节窗口降级,但不能擅自声明一个卷边界并把它当事实。

Volume summary 是派生对象。它必须带来源章节范围、摘要版本、生成时间、输入指纹和 reindex 健康状态;命中高风险改写时,summary 只能做导航和压缩,最终证据仍要回查作者文件或审批后事实。

## as-of chapter 语义

查询和上下文装配必须区分“全书最新事实”和“目标章节当时事实”。写作、改写、ReaderPanel 和一致性校验默认使用目标章节的 as-of chapter;只有用户明确问全书最新状态时,才返回最新事实。

| 场景 | 默认时点 |
|---|---|
| 改写第 N 章 | 第 N 章结束前后的项目状态,按目标段落位置决定。 |
| 续写第 N+1 章 | 第 N 章结尾状态。 |
| 查询角色当前状态 | 若来自正文选区,使用选区所在章节;若来自全局搜索,让用户切换“此章/最新”。 |
| 全书 cascade | 每个命中段落用自己的章节时点,最终审批解释再汇总最新影响。 |

S07 输出 context package 时必须写明 `as_of_chapter`、时点来源和被排除的未来事实。未来章节信息不能剧透式进入前文改写;若某个事实只能从未来章节证明,它只能作为冲突提示或低置信候选。
...
拒绝理由重做时,S07 必须把用户拒绝理由、旧 proposal 摘要、相似度判定输入和被拒条目的证据锚点装入 redo context。相似度判定不能只比表面文字;至少比较目标锚点、事实变化、风险类型、用户拒绝理由是否被解决和输出结构。S04 决定是否继续重做,S07 只提供可解释材料。
...
| cascade 体量与延迟 | 中等规模和全书级 cascade 的 token 体量、耗时、stream 心跳、reindex 影响和用户等待可被 preflight 解释 | cascade 改为分批执行、用户 checkpoint 和可中断队列,不能承诺一次性全书处理 |
...
查询结果必须带时点标签。默认 as-of 与任务上下文一致;用户可以切到“全书最新”。当结果来自 stale index、future-only evidence 或低置信语义召回时,结果项必须标记 stale/low-confidence,并提供跳回来源的路径。
...
S07 拥有 context 侧体量估算与批次建议,但不拥有审批批次决策。任何跨章节 cascade 在交给 S03 前,必须先形成 preflight 体量估算,让 S04 决定是一次提交、分批执行、要求用户确认范围与等待,还是因为证据不足而失败。

| 估算项 | S07 负责 | 交给谁 |
|---|---|---|
| context size | 估算每批必装事实、原文、摘要、召回 tail 和守则材料的体量 | S08 做 prompt packet 终局校验 |
| impact batch | 按 dependency group、章节范围、风险和可独立审批边界建议批次 | S04 决定审批批次和用户 checkpoint |
| uncertainty | 标记低置信候选、未验证能力 gate、索引 degraded 和 overflow 风险 | S04/S05 展示为 preflight 风险 |
| runtime estimate input | 输出模型调用次数、预计 reindex 范围、stream 心跳要求和可中断点 | I01 提供 provider 上限与限流,S04 生成用户级估算 |

如果 S07 无法给出可解释的 batch 和体量估算来源,不能把 cascade 包装成“正在处理全书”。它必须返回估算缺口或要求缩小范围。
...
Prompt 主权见 [S07](./S07-prompt-system.md),回放和 IO 记录见 [S09](./S09-llm-quality-harness.md),质量门禁见 [S10](./S10-evaluation-and-golden-regression.md)。
...
- [A04 · Tool Catalog](./appendix/A04-tool-catalog.md) 保存 analyzeImpact、assembleContext、queryFacts 等工具参数;工具权限和失败语义由 [S08](./S08-agent-tooling-boundary.md) 定义。
