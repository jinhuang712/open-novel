...
Discuss 默认也不学习作者偏好。只有作者显式说“记住这个”“以后都这样”“把这条当规则”这类记忆意图时,系统才可以生成经验候选;候选仍必须走 Reflector 的可见、去重和冲突处理,不能直接变成已确认经验。
...
| 识别显式“记住”意图并生成经验候选 | 从普通闲聊里自动学习偏好 |
...
pending approval 期间,Discuss 仍可作为只读讨论入口使用。作者可以追问“这批改动为什么影响第 18 章”“如果我拒绝第三项会怎样”或要求解释风险;系统只能回答、引用来源、打开相关位置,不能创建新 proposal、创建新 ChangeSet、修改当前审批卡、切换到可写模式或把讨论摘要写成项目事实。若用户在 Discuss 中说“那就顺手改掉”,Router 必须提示先处理当前审批或在审批结束后重新发起可写动作。
...
| 普通闲聊出现偏好 | 只作为本轮讨论材料 | 自动写入经验候选 |
| 显式要求记住 | 生成可见经验候选,等待 Reflector 去重/冲突处理 | 直接确认经验或覆盖旧经验 |
...
A: 不会自动学习。普通闲聊只留在本轮讨论里;只有你显式说“记住这个”“以后都这样”等,系统才生成经验候选。候选仍由 Reflector 去重、查冲突并展示给你处理,不会直接覆盖旧经验。
