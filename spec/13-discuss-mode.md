...
Discuss 的上下文包由 [Context And Query](./07-context-and-query.md) 提供。上下文不足时,Discuss 不能靠模型常识补成项目事实;它只能提出假设、追问或建议用户打开相关来源。
...
| 用户要求直接写入 | “需要切换到规划/写作并审批” | 在 Discuss 中创建 ChangeSet |
...
Discuss Mode 使用 [design/01 主界面](../design/01-main-layout.md) 的输入条;模式切换 UI 以 design 为视觉契约,行为边界以本篇和 [Turn Orchestration](./04-turn-orchestration.md) 为准。
...
A: 不能。它可以成为候选文本,但保存前必须切到规划/写作路径并审批。
