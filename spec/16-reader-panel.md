...
ReaderPanel 是发布前读者预演能力。根层 [Creative Engine](./08-creative-engine.md) 定义质量信号和守则;本篇定义读者报告如何生成、展示和进入审批解释。
...
它不替作者决定“该不该发”。它给出读者视角和风险解释,最终裁决仍归作者。
...
建议必须是可选择的,不能伪装成系统命令。进入写入路径时必须转入 [Approval Cascade](./15-approval-cascade.md)。

## Persona 边界

| 允许 | 不允许 |
|---|---|
| 追更党、逻辑控、情感党等视角 | 替作者裁决是否发布 |
| 标记弃读点和疑惑点 | 改写项目事实 |
...
[design/03 ReaderPanel](../design/03-reader-panel.md) 定义报告布局和可视化。行为主权以本篇和 [Creative Engine](./08-creative-engine.md) 为准。
