...
Turn Recap 回答另一类问题:“这一轮结束后,项目发生了什么,还能继续做什么”。Recap 可以显示在 Trace 面板顶部,但它不是 Trace step;完整语义见 [M17 · Turn Recap And Continuation](./M17-turn-recap-and-continuation.md)。
...
普通作者默认看到状态点和摘要。Developer Mode 才展示工具参数、JSON、用量明细等内部材料。
...
| Dev Diagnostic | 原始工具参数、JSON、用量 | Developer Mode |
| Recap | “已停止 · 没有修改 · 可继续剩余步骤” | 状态点 + Trace |
...
| turn 内部回滚或修正 | Trace 标明最终状态和影响范围 | 继续展示已落盘成功 |
| recap 缺失 | Trace 标记本轮缺少用户级回执 | 编造项目 changelog |
...
A: 通常不阻断。只有 Trace 失败暴露出业务状态不确定,例如最终写入状态无法确认,才升级为 turn 错误。
