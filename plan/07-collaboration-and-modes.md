...
**产品的回答**:一套铁三角协作契约——AI 只提议,你来审定,系统持久记账——加上三种界限分明的工作模式,让"它现在能动什么、正在干什么"在任何时刻一目了然。
...
**审定。** 每一个落笔的决定由你做出:通过、否决、改后通过(R1,见 [03 — 守则与红线](./03-guardrails.md))。小处措辞可以贴着正文就地确认,连带修改和高风险变更进入整批审批;否决时说出理由,AI 带着你的理由重做——这一来一回不是流程负担,是你调教这支团队的方式。

**持久。** 你通过之后,系统才把结果持久写入作品,并把这次决定记录在案:改了什么、依据什么、你怎么裁定的,事后随时可查;你的裁定本身也被沉淀为经验,让系统越用越懂你(见 [10 — 记忆与成长](./10-memory-and-learning.md))。

这条契约不分模式、不分角色、没有例外:润色师的一处措辞润色与写手的一整章正文,都必须让你先看见、先裁定;差别只在交互轻重。小改贴近正文确认,大改整批审批,再小的改动也不存在"小到不用审定"的后门。
...
| **规划** | 搭世界观、改设定 | 全部 | 提议修改设定(经你审批);只随批处理机械一致性正文项 |
| **写作** | 产出正文 | 全部 | 提议正文与连带改动(经你审批);不碰设定 |

三种模式眼界相同、权限不同:看,永远看得全——讨论时也能查任何一章,写作时也能引任何一条设定;改,各守一界——讨论一笔不动,规划不创作正文,写作不碰设定。界限分明带来的是确定感:你永远不必担心"聊着聊着它把稿子改了"。

**讨论,是想清楚之前的安全区。** 问"你觉得主角应该长什么样",查"林川上次见王小芳是哪一章",把一个拿不准的剧情走向先聊透——全程没有任何东西会被写入,聊得再深也不在作品上留痕。没有审批卡,因为没有什么需要审。

**规划,动世界、不创作故事。** 搭世界观、立角色、修改设定都在这里发生:写手起草设定提案,一致性守护者把改动波及的设定与章节一次找全,汇成审批卡等你裁定。若设定改动只牵出称谓、旧名、直接引用、事实性短替换这类机械一致性正文项,它们可以随同一张审批卡一起审;需要重写场面、补情绪、改对白走向或重塑读者感受的创作性正文修复,必须拆到写作模式。

**写作,产出正文的完整阵型。** [06 — AI 角色团队](./06-agent-team.md) 那条"写手起草、三路并行审、汇成一张审批卡"的流水线,就是写作模式的完整形态;讨论与规划只动用其中一部分,写作全员到齐。设定在这个模式下不被改动——写到一半发现设定本身要动,AI 会把它列成"需要先规划"的前置事项,相关正文提案停在待处理状态;你切到规划并审定前,系统不把正文和设定混成一批偷偷落笔。

把动设定与创作正文分进两个模式,不是分类洁癖:一处核心设定的改动波及面动辄几十处,若与正文产出搅在同一股劲里,你很快分不清哪一笔是创作、哪一笔是补救。一界管一事,每一次审批都只有一种心智负担。规划审批里允许的正文项只负责把已经被你审定的事实在文本中对齐;只要需要新的文学判断,它就进入写作模式。若正文改动必须依赖设定先改变,它就是规划前置,不是写作模式里的附赠设定修改。
...
AI 干活的全程,默认只占用你一句话的注意力:全部活动收敛为状态点旁的一行字。但收起的只是呈现层级,不是信息本身——全量工作日志、输入材料、动作记录、依据引用和可解释结论,随时可以召唤回看;觉得哪一步不对,这些可见证据可以原样复制下来发回给 AI 当反馈,让它确切知道自己错在哪一步。
