...
| **Writer** (吐字) | Pro | 生成正文 / 章节概要 / 设定文档 | readSetting, readChapter, listSettings, writeSetting✓, writeChapter✓, webSearch (mock) | 是 (写操作) |
| **Checker** (检查) | Flash | 风格、流畅度、节奏审阅 | readChapter, listSettings | 否 (只读) |
...
最终 → writeChapter tool (needsApproval=true)
        ↓
ApprovalCard (展示完整章节 + 高亮 Checker/Validator 标注的可疑段)
        ↓ 用户同意 → 落盘
        ↓
Reflector → 提炼经验
```
...
- **Flash 模型**: 速度+成本第一 → Router (调度本身不需深思)、Checker (轻量风格扫)、Reflector (短摘要)
