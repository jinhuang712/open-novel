...
Open Novel 的回答是一条铁规矩:**系统把主修改与全部连锁影响,在呈现给你之前全部算完。**连锁影响可达多级——改了 A 牵出 B,改 B 又牵出 C——系统沿着影响一路追到尽头,把所有波及汇成一份完整的修改全集。把波及一次找全的正是一致性守护者(见 [06 — AI 角色团队](./06-agent-team.md));你看到的永远是完整全貌,不是挤牙膏式的逐条弹窗。
...
**④ 守则风险区与读者预演区。**这次改动触碰的守则风险与模拟读者的反应在这里呈现;风险报告的内容见 [09 — 叙事诊断与读者预演](./09-narrative-and-reader.md),critical 与 blocking 的通过语义见 [03 — 守则与红线](./03-guardrails.md)。
...
4. **待审期间输入锁定。**审批卡停在你面前时,新输入与模式切换都暂时锁住,界面会明确告知原因:先把眼前的决定做完,新想法等审完再说(审批卡出现之前的排队语义见 [07 — 协作与三模式](./07-collaboration-and-modes.md))。这也是 R4 的体验面:世界没改一致之前,不继续写正文。
