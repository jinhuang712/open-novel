...
这些要素不是归档作业:它们直接喂给守则检测与弧光追踪(见 [03 — 守则与红线](./03-guardrails.md)、[10 — 叙事诊断与读者预演](./10-narrative-and-reader.md))——你填得越准,守护就越准。
...
- **全项目改名**:角色改名一次,全书每一处引用一起改,整批走审批(见 [07 — 审批与连带修改](./07-approval-and-cascade.md))——不存在改了资料、漏了正文的中间态。
- **框选修改**:框选一段正文,直接提要求;改完给前后对照,过审批才落笔。
...
- **重大改动自动留快照。** 核心设定的每次重大改动自动留底,可随时回退到改动之前。
...
- [spec/16 — 知识结构与派生守卫](../spec/16-knowledge-schema.md):六维故事世界的知识结构,R4 写作锁与 R7 派生资料守卫。
- [spec/17 — 段落锚点](../spec/17-paragraph-anchors.md):伏笔与呼应所依赖的段级稳定锚定。
- [spec/18 — 文意关联](../spec/18-embeddings.md):"相似桥段找得到彼此"的文意维度实现契约。
- [spec/19 — 影响分析](../spec/19-impact-analysis.md):设定改动连锁影响的确定性穷举(R6)。
- [spec/20 — 写前备料](../spec/20-context-assembly.md):写前自动备料的装配规则与 R9 失败语义。
- [spec/21 — 事实查询](../spec/21-fact-query.md):即问即答四类问题的查询契约。
- [spec/01 — 存储结构](../spec/01-storage-schema.md):角色卡、章节卡与设定体系的存储承载。
- [spec/05 — 实体高亮与跳转](../spec/05-entity-highlight.md):随点随跳、悬停摘要与引用回链的实现契约。
- [design/01 — 主界面](../design/01-main-layout.md):边写边查在稿面上的交互契约。
