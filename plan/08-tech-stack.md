...
> **[info]** 后续代码 commit 以本文档为依据。版本号实查清单见 [spec/00](../spec/00-version-audit.md)。
...
| `@ai-sdk/deepseek` | `2.0.x` | 锁主线 | model ID `deepseek-v4-pro` / `deepseek-v4-flash`;ctx 1M,max output 384K,原生 JSON mode(实查见 [spec/00 §C](../spec/00-version-audit.md)) |
...
**跨项目连接管理**:每个项目独立 `index.db` 文件;LRU(3) 缓存 3 个项目的 connection,切换项目时 evict 最旧的。详见 [plan/04 §连接池](./04-storage-model.md)。
...
**1M ctx 的设计含义**:普通章节场景下不需要 token 预算控制,把"一致性所需的全部上下文"装齐是头等优先级 — 见 [plan/12 §一致性优先](./12-memory-and-context.md) + [spec/23 §per-agent 上下文契约](../spec/23-context-contracts.md)。

**调用形态**(详见 [spec/24](../spec/24-json-output.md)):
...
不用任何 Agent 框架的 Memory 抽象,自己写薄薄一层 thread / resource CRUD + 一致性校验。详见 [spec/22 §应用层 memory 模块](../spec/22-memory-and-history.md)。
...
- Mastra Agent / Memory / Workflow 等高层抽象(被 P0-1 决策砍掉,详见 [ADR-A](#adr))

## 关联文档

- **上游**:[plan/01](./01-overview.md) 系统概览 §关键技术决策汇总
- **存储细节**:[plan/04](./04-storage-model.md) 存储模型 · [spec/01](../spec/01-storage-schema.md) SQLite schema
- **实查清单**:[spec/00](../spec/00-version-audit.md) 版本审计闸门
- **DeepSeek 适配**:[spec/22](../spec/22-memory-and-history.md) §DeepSeek middleware
...
| ADR-C | 向量能力 | LibSQL native vector / **sqlite-vec** / sqlite-vss / hnswlib-node / Vectra | **sqlite-vec** | 与 SQLite 同库,可 SQL JOIN([spec/20](../spec/20-context-assembly.md) / [spec/21](../spec/21-fact-query.md) 范式契合);transaction 一致性免费;hnswlib-node / Vectra 都要"两次查询"+ JS merge,与现有 spec 范式不符 |
