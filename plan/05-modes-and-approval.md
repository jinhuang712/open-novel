...
> **proposal + 独立 endpoint 模式**: 工具执行时不真正写盘,而是把 proposal 落 `approvals` 表 (status=pending) 立即返回;Agent loop 看到 proposal 后自然结束 stream;用户审批通过独立 endpoint `POST /api/approvals/{id}/resolve` 真正落盘。审批悬挂不依赖 stream 长连接。详见 [spec/06-approval-flow.md](../spec/06-approval-flow.md)。
